(() => {
  ...
  window.SpriteIndexAccount.register("notifications", function initializeAccountFeature() {
    ...
    function fillTimezoneSelect(selectEl, current) {
      ...
      selectEl.innerHTML = sorted.map((tz) => `<option value="${escapeHtml(tz)}">${escapeHtml(tz)}</option>`).join("");
    ...
    function applyNotificationSettings(prefs) {
      ...
      fillTimezoneSelect(document.getElementById("notifTimezone"), prefs.timeZone || prefs.timezone || "Europe/Paris");
    ...
    if (notifSettingsRoot) {
      notifSettingsRoot.addEventListener("change", (e) => {
        if (
          !e.target.closest(
            "[data-notif-channel], [data-notif-type], [data-notif-frequency], [data-notif-in-app], [data-notif-push], #notifQuietStart, #notifQuietEnd, #notifTimezone, #notifMaxPushPerDay"
          )
        ) {
          return;
        }
    ...
    Object.assign(globalThis, {
      ...
      collectNotificationSettingsPayload,
      saveNotificationSettings,
      updateTopbarAvatar
    });
  });
