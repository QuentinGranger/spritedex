(() => {
  ...
  window.SpriteIndexAccount.register("passport-loading", function initializeAccountFeature() {
    async function fetchCollectorPassport(userId) {
      const res = await fetch(`${API_BASE}/profile/${encodeURIComponent(userId)}/passport`, {
    ...
    async function openCollectorPassport(userId, displayName = "") {
      ...
      if (titleEl)
        titleEl.textContent = displayName
          ? t("account.passportTitleUser", { name: displayName })
    ...
    function wirePassportBodyActions(content, data = null) {
      ...
      const name =
        (passportData.user && (passportData.user.displayName || passportData.user.username)) || t("account.player");
      ...
      content.querySelectorAll('[data-passport-action="compare-user"]').forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          if (typeof compareWithFriend === "function") {
            await compareWithFriend(btn.dataset.id || ownerId, btn.dataset.name || name, { source: "passport" });
          }
        });
      });
      ...
      content.querySelectorAll('[data-passport-action="pin-badge"]').forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          btn.disabled = true;
          try {
            const save = await fetch(`${API_BASE}/profile/${encodeURIComponent(state.userId)}/passport/settings`, {
              method: "PATCH",
              headers: authHeaders(),
    ...
    async function loadCollectorPassportSettings(content) {
      ...
      try {
        const res = await fetch(`${API_BASE}/profile/${encodeURIComponent(state.userId)}/passport/settings`, {
          headers: authHeadersOnly()
        });
        if (!res.ok) return;
        const settings = await res.json();
        const options = (selected) =>
          ["private", "friends", "squad", "public"]
            .map(
              (value) =>
            ...
            .join("");
        const squadOptions = [
          `<option value="">${t("account.passport.settings.noSquad")}</option>`,
          ...(settings.availableSquads || []).map(
            (s) =>
              `<option value="${escapeHtml(String(s.id))}" ${String(settings.primarySquadId) === String(s.id) ? "selected" : ""}>${escapeHtml(s.name)}</option>`
          )
        ].join("");
        const featuredOptions = [
          `<option value="">${t("account.passport.settings.noBadge")}</option>`,
          ...(settings.availableFeaturedBadges || []).map(
            (b) =>
        ...
        saveBtn.addEventListener("click", async () => {
          ...
          content.querySelectorAll("[data-passport-setting]").forEach((field) => {
            if (field.type === "checkbox") {
              ...
              field.dataset.passportSetting === "primarySquadId" ||
              field.dataset.passportSetting === "featuredBadgeId"
            ) {
    ...
    Object.assign(globalThis, {
      fetchCollectorPassport,
      loadCollectorPassport,
      openCollectorPassport,
      wirePassportBodyActions,
      loadCollectorPassportSettings
    });
  });
})();
