  ...
  window.SpriteIndexAccount.register("passport-share-preview", function initializeAccountFeature() {
    ...
    function buildPassportCardLines(card, opts) {
      ...
      if (opts.showCompletion && card.ownedVariantCount != null && card.releasedVariantCount != null) {
        lines.push({
          kind: "stat",
          text: t("account.share.variantsOf", { owned: card.ownedVariantCount, total: card.releasedVariantCount })
        });
      }
      if (opts.showEvents && card.completedEventCount != null) {
        ...
        lines.push({
          kind: "stat",
          text: n === 1 ? t("account.passport.eventsShareOne") : t("account.passport.eventsShareMany", { count: n })
        });
      }
      ...
      if (opts.showJoinedAt && card.joinedAt) {
        lines.push({
          kind: "meta",
          text: t("account.share.joinedOn", { date: formatPassportJoinDate(card.joinedAt) })
        });
      }
    ...
    function showPassportShareResult(result) {
      ...
      if (nativeShare) {
        ...
        nativeShare.onclick = nativeSupported
          ? async () => {
              try {
    ...
    function openPassportSharePreview(passportData) {
      ...
      fetchPassportCardPayload(username)
        .then((card) => {
          ...
          [
            "passportShareOptSquad",
            "passportShareOptBadges",
            "passportShareOptJoined",
            "passportShareOptCompletion",
            "passportShareOptEvents",
            "passportShareOptInvite"
          ...
          if (generateBtn) {
            generateBtn.onclick = async () => {
              ...
              try {
                ...
                const result = await generateAndSharePassportCard(
                  card,
                  opts,
                  format,
                  opts.includeInvite ? invitation : null
                );
    ...
    Object.assign(globalThis, {
      passportShareDefaults,
      formatPassportJoinDate,
      buildPassportCardLines,
      renderPassportSharePreviewBody,
      fetchPassportCardPayload,
      resetPassportShareResult,
      showPassportShareResult,
      openPassportSharePreview,
      passportCardSize,
      createPassportShareInvitation,
      passportShareUrl
    });
  });
})();
