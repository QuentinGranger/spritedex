(() => {
  ...
  async function openCollectorPassportByUsername(username, displayName = "") {
    ...
    try {
      ...
      if (
        state.userId &&
        userId &&
        String(userId).match(/^\d+$/) &&
        typeof window.openCollectorPassport === "function"
      ) {
  ...
  function renderPublicPassportOverlay(normalized) {
    ...
    const rate =
      stats.completionRateDisplay != null
    ...
    try {
      invitationPending = !!sessionStorage.getItem("sprite-index_pending_friend_invite");
    } catch (_) {
      /* storage unavailable */
    }
    const actionLabels = {
        ...
        <div class="shared-view__overall-top">
          ...
          <span class="shared-view__overall-count">${
            stats.ownedVariantCount != null && stats.releasedVariantCount != null
              ? t("account.passport.variantCount", {
                  owned: stats.ownedVariantCount,
                  total: stats.releasedVariantCount
                })
              : ""
          }</span>
        </div>
    ...
    overlay.querySelectorAll("[data-public-passport-action]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (action === "add_friend") {
          ...
          if (!state.userId) {
            toast(t("account.loginToCompare"));
            return;
          }
