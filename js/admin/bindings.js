(() => {
  ...
  document.addEventListener("click", (event) => {
    if (event.target.closest("#adminUniversalSearch")) {
      openUniversalSearch();
      return;
    }
    const universalFilter = event.target.closest("[data-universal-filter]");
    if (universalFilter) {
      state.universalSearch.activeGroup = universalFilter.dataset.universalFilter;
      state.universalSearch.activeIndex = 0;
      renderUniversalSearch();
      return;
    }
    const universalResult = event.target.closest("[data-universal-result]");
    if (universalResult) {
      openUniversalResult(universalResult);
      return;
    }
    const tab = event.target.closest("[data-admin-tab]");
    if (tab) {
      setTab(tab.dataset.adminTab);
      return;
    }
    ...
    if (openPlayer) {
      const open = () =>
        selectPlayer(openPlayer.dataset.openPlayer, {
    ...
    const reportAppeal = event.target.closest("[data-report-appeal]");
    if (reportAppeal) {
      handleReportAppeal(reportAppeal);
      return;
    }
    const reportPriority = event.target.closest("[data-report-priority-filter]");
    if (reportPriority) {
      state.players.reportPriority =
        state.players.reportPriority === reportPriority.dataset.reportPriorityFilter
          ? "all"
          : reportPriority.dataset.reportPriorityFilter;
      loadTab("players", true);
      return;
    }
    const catalog = event.target.closest("[data-catalog-id]");
    if (catalog) {
      selectCatalog(catalog.dataset.catalogId);
      return;
    }
    if (event.target.closest("#eventsBulkSelectPage")) {
      toggleBulkPage("events", event.target.checked);
      return;
    }
    if (event.target.closest("[data-notification-select-page]")) {
      const allSelected =
        state.notifications.visibleItems.length &&
        state.notifications.visibleItems.every((job) => state.notifications.bulkIds.has(String(job.id)));
      state.notifications.visibleItems.forEach((job) => {
        if (allSelected) state.notifications.bulkIds.delete(String(job.id));
        else state.notifications.bulkIds.add(String(job.id));
      });
      loadTab("notifications", true);
      return;
    }
    if (event.target.closest("[data-notification-bulk-apply]")) {
      openBulkAction("notifications");
      return;
    }
    const notificationToggle = event.target.closest("[data-bulk-notification-toggle]");
    if (notificationToggle) {
      notificationToggle.checked
        ? state.notifications.bulkIds.add(notificationToggle.dataset.bulkNotificationToggle)
        : state.notifications.bulkIds.delete(notificationToggle.dataset.bulkNotificationToggle);
      loadTab("notifications", true);
      return;
    }
    if (event.target.closest("[data-collection-select-page]")) {
      const allSelected =
        state.collections.visibleItems.length &&
        state.collections.visibleItems.every((entry) => state.collections.bulkIds.has(String(entry.id)));
      state.collections.visibleItems.forEach((entry) => {
        if (allSelected) state.collections.bulkIds.delete(String(entry.id));
        else state.collections.bulkIds.add(String(entry.id));
      });
      loadTab("collections", true);
      return;
    }
    if (event.target.closest("[data-collection-bulk-apply]")) {
      openBulkAction("collections");
      return;
    }
    const collectionToggle = event.target.closest("[data-bulk-collection-toggle]");
    if (collectionToggle) {
      collectionToggle.checked
        ? state.collections.bulkIds.add(collectionToggle.dataset.bulkCollectionToggle)
        : state.collections.bulkIds.delete(collectionToggle.dataset.bulkCollectionToggle);
      loadTab("collections", true);
      return;
    }
    const catalogToggle = event.target.closest("[data-bulk-catalog-toggle]");
    if (catalogToggle) {
      catalogToggle.checked
        ? state.catalog.bulkIds.add(catalogToggle.dataset.bulkCatalogToggle)
        : state.catalog.bulkIds.delete(catalogToggle.dataset.bulkCatalogToggle);
      renderBulkBar("catalog");
      return;
    }
    const eventToggle = event.target.closest("[data-bulk-event-toggle]");
    if (eventToggle) {
      eventToggle.checked
        ? state.events.bulkIds.add(eventToggle.dataset.bulkEventToggle)
        : state.events.bulkIds.delete(eventToggle.dataset.bulkEventToggle);
      renderBulkBar("events");
      return;
    }
    if (event.target.closest("#catalogBulkApply")) {
      openBulkAction("catalog");
      return;
    }
    if (event.target.closest("#eventsBulkApply")) {
      openBulkAction("events");
      return;
    }
    if (event.target.closest("[data-catalog-show-issues]")) {
      $("#catalogState").value = "data_issues";
      state.catalog.page = 1;
      loadTab("catalog", true);
      return;
    }
    if (event.target.closest("#catalogBulkClear")) {
      state.catalog.bulkIds.clear();
      loadTab("catalog", true);
      return;
    }
    if (event.target.closest("#eventsBulkClear")) {
      state.events.bulkIds.clear();
      loadTab("events", true);
      return;
    }
    const workflow = event.target.closest("[data-catalog-workflow]");
    if (workflow) {
      setCatalogWorkflow(workflow);
      return;
    }
    const variantWorkflow = event.target.closest("[data-variant-workflow]");
    if (variantWorkflow) {
      setVariantWorkflow(variantWorkflow);
      return;
    }
    const rollback = event.target.closest("[data-catalog-rollback]");
    if (rollback) {
      rollbackCatalogHistory(rollback);
      return;
    }
    const variant = event.target.closest("[data-edit-variant]");
    if (variant) {
      openVariantEditor(variant);
      return;
    }
    const availability = event.target.closest("#addAvailability");
    if (availability) {
    ...
    const newsEdit = event.target.closest("[data-edit-news]");
    if (newsEdit) {
      editNews(newsEdit);
      return;
    }
    const news = event.target.closest("[data-news-action]");
    if (news) {
    ...
    if (cancelInvite) {
      openSquadInviteCancelDialog(cancelInvite);
      return;
    }
    if (event.target.closest("[data-close-squad]")) {
      state.social.selected = null;
      renderSquadDossierEmpty();
      $("#squadsList")
        ?.querySelectorAll("tr[data-squad-id]")
        .forEach((node) => node.classList.remove("is-selected"));
      return;
    }
    const squad = event.target.closest("[data-squad-toggle]");
    if (squad) {
      openSquadAccessDialog(squad);
      return;
    }
    ...
    const scope = event.target.closest("[data-passport-scope]");
    if (scope) {
      queuePassports(scope.dataset.passportScope);
      return;
    }
    const page = event.target.closest("[data-page-kind]");
    if (page && !page.disabled) {
      state[page.dataset.pageKind].page = Number(page.dataset.page);
      loadTab(page.dataset.pageKind, true);
      return;
    }
    const privacyExport = event.target.closest("[data-privacy-export]");
    if (privacyExport) {
      openPrivacyExportDialog({
        userId: privacyExport.dataset.privacyExport,
        label: privacyExport.dataset.privacyLabel || ""
      });
      return;
    }
    ...
    if (privacyRestore) {
      openPrivacyRestoreDialog(privacyRestore);
      return;
    }
    ...
    if (event.target.closest("#createAdminOperator")) {
      openAdminOperatorDialog();
      return;
    }
    const operatorRotate = event.target.closest("[data-operator-rotate]");
    if (operatorRotate) {
      openAdminOperatorDialog({
        operatorId: operatorRotate.dataset.operatorRotate,
        name: operatorRotate.dataset.operatorName || ""
      });
      return;
    }
    const operatorToggle = event.target.closest("[data-operator-toggle]");
    if (operatorToggle) {
      toggleAdminOperator(operatorToggle);
      return;
    }
    const securityAlert = event.target.closest("[data-security-alert-ack]");
    if (securityAlert) {
      acknowledgeSecurityAlert(securityAlert);
      return;
    }
    if (event.target.closest("#retryFailedNotifications")) {
      retryFailedNotifications();
      return;
    }
    const auditDetail = event.target.closest("[data-audit-detail]");
    if (auditDetail) {
      openAuditDetail(auditDetail.dataset.auditDetail);
      return;
    }
    const auditRange = event.target.closest("[data-audit-range]");
    if (auditRange) {
      setAuditRange(auditRange.dataset.auditRange);
      return;
    }
    ...
    if (event.target.closest("#auditExportButton")) {
      exportAudit();
      return;
    }
    if (event.target.closest("#auditResetButton")) {
      state.audit = { ...state.audit, page: 1, q: "", action: "", targetType: "", from: "", to: "", range: "all" };
      ["#auditSearch", "#auditFrom", "#auditTo"].forEach((id) => {
        if ($(id)) $(id).value = "";
      });
      ["#auditActionFilter", "#auditTargetFilter"].forEach((id) => {
        if ($(id)) $(id).value = "";
      });
      loadAudit().catch((error) => setAlert(error.message || tr("loadFailed")));
      return;
    ...
    if (revoke) {
      request(
        `/api/admin/sessions/${encodeURIComponent(revoke.dataset.revokeSession)}`,
        { method: "DELETE" },
        { refresh: "privacy" }
      );
      return;
    }
    if (event.target.closest("#createEvent")) {
      createEvent();
      return;
    }
    if (event.target.closest("#createNews")) {
      createNews();
      return;
    }
    if (event.target.closest("#repairSpriteReferences")) {
      openBulkAction("collections");
      return;
    }
  ...
  document.addEventListener("change", (event) => {
    ...
    if (
      event.target.id === "auditActionFilter" ||
      event.target.id === "auditTargetFilter" ||
      event.target.id === "auditFrom" ||
      event.target.id === "auditTo"
    ) {
  ...
  document.addEventListener("input", (event) => {
    ...
    if (event.target.id === "auditSearch") {
      ...
      state.audit.searchTimer = setTimeout(
        () => loadAudit().catch((error) => setAlert(error.message || tr("loadFailed"))),
        220
      );
      return;
  ...
  document.addEventListener("submit", (event) => {
    if (event.target.id === "catalogEditForm") {
      event.preventDefault();
      saveCatalogForm(event.target);
    }
  ...
  $("#adminSearchDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
  ...
  $("#adminSearchInput")?.addEventListener("keydown", (event) => {
    if (event.key === "ArrowDown") {
      event.preventDefault();
      moveUniversalSelection(1);
    } else if (event.key === "ArrowUp") {
      event.preventDefault();
      moveUniversalSelection(-1);
    } else if (event.key === "Enter") {
      const active = document.querySelector(
        `.admin-search-result[data-universal-index="${state.universalSearch.activeIndex}"]`
      );
      if (active) {
        event.preventDefault();
        openUniversalResult(active);
      }
    }
  });
  ...
  $("#reportPriorityFilter").addEventListener("change", (event) => {
    state.players.reportPriority = event.target.value;
    loadTab("players", true);
  });
  ...
  $("#privacyPurgeDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closePrivacyPurgeDialog();
  });
  [
    ["#adminOperatorClose", closeAdminOperatorDialog],
    ["#adminOperatorCancel", closeAdminOperatorDialog]
  ].forEach(([id, handler]) => $(id)?.addEventListener("click", handler));
  $("#adminOperatorDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closeAdminOperatorDialog();
  });
  [
    ["#bulkActionClose", closeBulkAction],
    ["#bulkActionCancel", closeBulkAction]
  ].forEach(([id, handler]) => $(id)?.addEventListener("click", handler));
  $("#bulkActionDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closeBulkAction();
  });
  $("#privacyExportDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closePrivacyExportDialog();
  });
  $("#privacyRestoreDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closePrivacyRestoreDialog();
  });
  $("#privacyRevokeLinksDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
  ...
  $("#adminReasonDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
  ...
  [
    ["#auditDetailClose", closeAuditDetail],
    ["#auditDetailDone", closeAuditDetail]
  ].forEach(([id, handler]) => $(id)?.addEventListener("click", handler));
  $("#auditDetailDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closeAuditDetail();
  });
  $("#squadSearchButton")?.addEventListener("click", () => {
    state.social.page = 1;
    loadTab("social", true);
  });
  ...
  $("#squadJoinFilter")?.addEventListener("change", () => {
    state.social.page = 1;
    loadTab("social", true);
  });
  [
  ...
  $("#squadAccessDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closeSquadAccessDialog();
  });
  $("#squadInviteCancelDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
  ...
  $("#catalogSearchButton").addEventListener("click", () => {
    ...
    $("#catalogEditor").innerHTML = empty(tr("catalogPick", "Sélectionnez un sprite."));
    loadTab("catalog", true);
  });
  ...
  $("#playerSuspensionDialog").addEventListener("cancel", (event) => {
    event.preventDefault();
  ...
  $("#reportDecisionDialog").addEventListener("cancel", (event) => {
    event.preventDefault();
  ...
  $("#revokeOtherSessions")?.addEventListener("click", async () => {
    ...
    if (result)
      setNotice(
        english
          ? `${formatNumber(result.revoked)} other session(s) revoked.`
          : `${formatNumber(result.revoked)} autre(s) session(s) révoquée(s).`
      );
  });
