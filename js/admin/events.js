  ...
  async function loadEvents() {
    ...
    $("#eventsList").innerHTML = events.items.length
      ? events.items
          ...
          <td>${stateBadge(item.data_status || tr("unknown"), item.data_status === "complete" || item.data_status === "verified" ? "good" : "warning")}</td>
      ...
      ? news.items
          ...
          <div class="admin-news__body">
            <div class="admin-news__top">
              ...
              ${stateBadge(label(item.status), item.status === "published" ? "good" : item.status === "draft" ? "warning" : "")}
            </div>
            <p>${escapeHtml(item.description || "")}</p>
            <div class="admin-news__actions">
              ${
                can("events.write")
                  ? `<button class="admin-row-button" type="button" data-edit-news="${item.id}">${tr("edit", "Modifier")}</button>
