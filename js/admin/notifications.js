(() => {
  ...
  async function loadNotifications() {
    ...
    const queue = new Map((data.queue || []).map((row) => [row.status, row.count])),
      push = data.push || {},
      digests = data.digests || {},
      health = data.health || {};
    $("#notificationKpis").innerHTML = [
      kpi(
        english ? "Queued deliveries" : "Livraisons en attente",
        formatNumber((Number(queue.get("pending")) || 0) + (Number(queue.get("processing")) || 0)),
        english ? "push and email" : "push et e-mail"
      ),
      kpi(
        english ? "Failed deliveries" : "Livraisons en échec",
        formatNumber(queue.get("failed")),
        english ? "recoverable jobs" : "jobs récupérables",
        Number(queue.get("failed")) ? "danger" : ""
      ),
      kpi(
        english ? "Active push devices" : "Appareils push actifs",
        formatNumber(push.active),
        `${formatNumber(push.invalid)} ${english ? "invalid" : "invalides"}`
      ),
      kpi(
        english ? "Digest queue" : "File des digests",
        formatNumber(digests.count),
        digests.next_flush_at ? `${english ? "next" : "prochain"} ${formatDate(digests.next_flush_at)}` : "—"
      )
    ].join("");
    $("#notificationDeliveries").innerHTML = (data.deliveries || []).length
      ? data.deliveries
          .map(
            (row) =>
              `<div class="admin-status-row"><span>${escapeHtml(label(row.channel))} · ${escapeHtml(label(row.status))}</span><strong>${formatNumber(row.count)}</strong></div>`
          )
          .join("")
      : empty();
    $("#notificationHealth").innerHTML = [
      [
        english ? "Oldest queued" : "Plus ancienne en attente",
        health.oldest_pending_at ? formatDate(health.oldest_pending_at) : "—"
      ],
      [
        english ? "Latest failure" : "Dernier échec",
        health.latest_failure_at ? formatDate(health.latest_failure_at) : "—"
      ],
      [english ? "Cancelled" : "Annulées", formatNumber(health.cancelled)]
    ]
      .map(
        ([key, value]) =>
          `<div class="admin-status-row"><span>${escapeHtml(key)}</span><strong>${escapeHtml(value)}</strong></div>`
      )
      .join("");
