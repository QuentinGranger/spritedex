(() => {
  ...
  async function repairReferences() {
    const why = await requestReason(
      english
        ? "Why run the safe sprite-reference repair?"
        : "Pourquoi exécuter la correction sûre des références Sprite ?"
    );
    if (!why) return;
    await request(
      "/api/admin/collections/integrity/repair",
      {
        ...
        body: JSON.stringify({ action: "backfill-sprite-references", reason: why })
      },
      { refresh: "collections" }
    );
  }
  function openSquadAccessDialog(button) {
    ...
    $("#squadAccessTitle").textContent = opening
      ? english
        ? "Reopen public joining"
        : "Rouvrir l’accès public"
      : english
        ? "Close public joining"
        : "Fermer l’accès public";
    $("#squadAccessSummary").textContent = `${state.squadAccess.name}`;
    $("#squadAccessImpact").textContent = opening
      ? english
        ? "Anyone with the code will be able to join this squad again."
        : "Toute personne disposant du code pourra rejoindre cette squad."
      : english
        ? "New joins via code will be blocked until the access is reopened."
        : "Les nouvelles inscriptions via code seront bloquées jusqu’à réouverture.";
  ...
  async function submitSquadAccess(event) {
    ...
    try {
      ...
      setNotice(
        operation.open
          ? english
            ? "Public joining reopened."
            : "Accès public rouvert."
          : english
            ? "Public joining closed."
            : "Accès public fermé."
      );
  ...
  async function retryNotification(button) {
    const why = await requestReason(english ? "Why retry this delivery?" : "Pourquoi relancer cette livraison ?");
    if (!why) return;
    await request(
      `/api/admin/notifications/queue/${button.dataset.retryJob}/retry`,
      { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reason: why }) },
      { refresh: "notifications" }
    );
  }
  async function processNotifications() {
    const why = await requestReason(english ? "Why process the queue now?" : "Pourquoi traiter la file maintenant ?");
    if (!why) return;
    await request(
  ...
  async function queuePassports(scope) {
    const why = await requestReason(
      scope === "all"
        ? english
          ? "Why queue all passports?"
          : "Pourquoi planifier tous les passeports ?"
        : english
          ? "Why queue stale passports?"
          : "Pourquoi planifier les passeports obsolètes ?"
    );
    if (!why) return;
    await request(
      "/api/admin/passports/recalculate",
      { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scope, reason: why }) },
      { refresh: "passports" }
    );
  }
  async function setMetricFlag(input) {
    ...
    const why = await requestReason(
      disabled
        ? english
          ? `Why suspend ${key}?`
          : `Pourquoi suspendre ${key} ?`
        : english
          ? `Why restore ${key}?`
          : `Pourquoi rétablir ${key} ?`
    );
    if (!why) {
  ...
  async function exportAggregates() {
    try {
      ...
      link.href = url;
      link.download = english ? "sprite-index-aggregates.json" : "sprite-index-agregats.json";
      link.click();
  ...
  Object.assign(window, {
    repairReferences,
    openSquadAccessDialog,
    closeSquadAccessDialog,
    submitSquadAccess,
    openSquadInviteCancelDialog,
    closeSquadInviteCancelDialog,
    submitSquadInviteCancel,
    retryNotification,
