(() => {
  ...
  async function loadPrivacy() {
    ...
    $("#privacyKpis").innerHTML = [
      kpi(
        english ? "Deletion queue" : "File de suppression",
        formatNumber(p.deletion_requests),
        `${formatNumber(summary.ready ?? p.ready_for_purge)} ${english ? "ready" : "prêtes"} · ${formatNumber(summary.pending || 0)} ${english ? "in retention" : "en rétention"}`,
        Number(p.ready_for_purge) ? "warning" : ""
      ),
      kpi(
        english ? "Retention" : "Rétention",
        `${formatNumber(p.retentionDays || deletions.retentionDays || 30)} ${english ? "days" : "j"}`,
        english ? "before permanent purge" : "avant purge définitive"
      ),
      kpi(
        english ? "Active share links" : "Liens actifs",
        formatNumber(
          (Number(s.passport_links) || 0) + (Number(s.compare_links) || 0) + (Number(s.friend_invite_links) || 0)
        ),
        `${formatNumber(s.passport_links)} / ${formatNumber(s.compare_links)} / ${formatNumber(s.friend_invite_links)}`,
        (Number(s.passport_links) || 0) + (Number(s.compare_links) || 0) + (Number(s.friend_invite_links) || 0)
          ? ""
          : "good"
      ),
      kpi(
        english ? "Admin sessions" : "Sessions admin",
        formatNumber((data.sessions || []).length),
        `${data.roles?.role || "—"} · ${data.roles?.actor || "—"}`
      )
    ].join("");
    ...
    $("#privacyDeletionQueue").innerHTML = (deletions.items || []).length
      ? deletions.items
          ...
          <td>${
            ready
              ? stateBadge(english ? "Ready" : "Prêt", "warning")
              : stateBadge(
                  english
                    ? `${formatNumber(item.daysUntilPurge)}d left`
                    : `${formatNumber(item.daysUntilPurge)} j restants`,
                  ""
                )
          }</td>
    ...
    $("#privacyConsent").innerHTML =
      (data.consentVersions || [])
        .map(
          (item) =>
            `<div class="admin-status-row"><span>${escapeHtml(item.version)}</span><strong>${formatNumber(item.count)}</strong></div>`
        )
        .join("") || empty();
    $("#privacySharing").innerHTML = [
      [english ? "Passport links" : "Liens passeport", s.passport_links],
      [english ? "Comparison links" : "Liens de comparaison", s.compare_links],
      [english ? "Friend invitation links" : "Liens d’invitation", s.friend_invite_links]
    ]
      .map(
        ([name, value]) =>
          `<div class="admin-status-row"><span>${escapeHtml(name)}</span><strong>${formatNumber(value)}</strong></div>`
      )
      .join("");
    $("#privacySessions").innerHTML = (data.sessions || []).length
      ? data.sessions
          .map((item) => {
            ...
            const meta = [item.lastIp || item.createdIp || "—", formatDate(item.lastSeenAt || item.createdAt)].join(
              " · "
            );
    ...
    if (operatorsNode) {
      operatorsNode.innerHTML = (accessSecurity.operators || []).length
        ? accessSecurity.operators
            .map((item) => {
              const current = item.id === state.session?.operatorId;
              const initials = String(item.displayName || item.username || "A")
                .trim()
                .slice(0, 2)
                .toUpperCase();
              const lastLogin = item.lastLoginAt
                ? `${english ? "Last sign-in" : "Dernière connexion"} ${formatDate(item.lastLoginAt)}`
                : english
                  ? "Never signed in"
                  : "Jamais connecté";
        ...
        : empty(
            english
              ? "No named administrator yet. Create one before retiring the global secret."
              : "Aucun administrateur nominatif. Créez-en un avant de retirer le secret global."
          );
    }
    const alertsNode = $("#adminSecurityAlerts");
    if (alertsNode) {
      alertsNode.innerHTML = (accessSecurity.alerts || []).length
        ? accessSecurity.alerts
            .map(
              (alert) =>
                `<div class="admin-status-row admin-status-row--alert"><div><strong>${escapeHtml(alert.kind === "unusual_login" ? (english ? "Unusual connection" : "Connexion inhabituelle") : label(alert.kind))} <small>${escapeHtml(alert.username ? `@${alert.username}` : "—")}</small></strong><small>${formatDate(alert.created_at)}${alert.details?.ip ? ` · ${escapeHtml(alert.details.ip)}` : ""}${alert.acknowledged_at ? ` · ${english ? "reviewed" : "traitée"}` : ""}</small></div>${alert.acknowledged_at ? stateBadge(english ? "Reviewed" : "Traitée", "good") : `<button class="admin-row-button" type="button" data-security-alert-ack="${escapeHtml(alert.id)}">${english ? "Acknowledge" : "Traiter"}</button>`}</div>`
            )
            .join("")
        : empty(english ? "No unusual connection alert." : "Aucune alerte de connexion inhabituelle.");
    }
