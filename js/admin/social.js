(() => {
  ...
  async function loadSocial() {
    ...
    $("#socialKpis").innerHTML = [
      kpi(
        english ? "Squads" : "Squads",
        formatNumber(s.squads),
        `${formatNumber(s.open_join_squads)} ${english ? "open join" : "accès ouvert"} · ${formatNumber(s.active_members)} ${english ? "members" : "membres"}`
      ),
      kpi(
        english ? "Activity · 24h" : "Activité · 24 h",
        formatNumber(s.activity24h),
        english ? "squad events" : "événements de squad"
      ),
      kpi(
        english ? "Friendships" : "Amitiés",
        formatNumber(s.friendships),
        `${formatNumber(s.pending_friendships)} ${english ? "pending" : "en attente"}`,
        Number(s.pending_friendships) ? "warning" : ""
      ),
      kpi(
        english ? "Squad invites" : "Invitations",
        formatNumber(s.pending_squad_invitations),
        english ? "pending" : "en attente",
        Number(s.pending_squad_invitations) ? "warning" : ""
      ),
      kpi(
        english ? "Wishes / blocks" : "Souhaits / blocages",
        `${formatNumber(s.wanted_items)} / ${formatNumber(s.blocks)}`,
        english ? "wanted · safety" : "à trouver · sûreté",
        Number(s.blocks) ? "warning" : ""
      )
    ].join("");
    ...
    $("#squadsList").innerHTML = squads.items.length
      ? squads.items
          .map((squad) => {
            ...
            const stale =
              !squad.last_activity_at ||
              Date.now() - new Date(squad.last_activity_at).getTime() > 30 * 24 * 60 * 60 * 1000;
    ...
    $("#socialActivity").innerHTML =
      (data.activity24h || [])
        .map(
          (row) =>
            `<span class="admin-activity-chip">${escapeHtml(label(row.type))} · ${formatNumber(row.count)}</span>`
        )
        .join("") || empty(english ? "No squad activity in 24h." : "Aucune activité squad sur 24 h.");
    $("#socialPendingInvites").innerHTML = (data.pendingInvites || []).length
      ? data.pendingInvites
          .map(
            (item) =>
              `<div class="admin-status-row"><span><strong>@${escapeHtml(item.invitee_username)}</strong> → ${escapeHtml(item.squad_name)} <small>@${escapeHtml(item.inviter_username)} · ${formatDate(item.created_at)}</small></span><div class="admin-row-actions"><button class="admin-row-button" type="button" data-open-squad="${escapeHtml(item.squad_id)}">${english ? "Squad" : "Squad"}</button>${can("social.write") ? `<button class="admin-row-button admin-row-button--danger" type="button" data-cancel-invite="${escapeHtml(item.id)}" data-invite-label="@${escapeHtml(item.invitee_username)} → ${escapeHtml(item.squad_name)}">${english ? "Cancel" : "Annuler"}</button>` : ""}</div></div>`
          )
          .join("")
    ...
    $("#socialPendingFriends").innerHTML = (data.pendingFriendships || []).length
      ? data.pendingFriendships
          .map(
            (item) =>
              `<div class="admin-status-row"><span><strong>@${escapeHtml(item.requester_username)}</strong> → @${escapeHtml(item.addressee_username)}<small>${formatDate(item.created_at)}</small></span><div class="admin-row-actions"><button class="admin-row-button" type="button" data-open-player="${escapeHtml(item.requester_id)}">${english ? "From" : "De"}</button><button class="admin-row-button" type="button" data-open-player="${escapeHtml(item.addressee_id)}">${english ? "To" : "Vers"}</button></div></div>`
          )
          .join("")
    ...
    $("#socialRecentBlocks").innerHTML = (data.recentBlocks || []).length
      ? data.recentBlocks
          .map(
            (item) =>
              `<div class="admin-status-row"><span><strong>@${escapeHtml(item.blocker_username)}</strong> → @${escapeHtml(item.blocked_username)}<small>${escapeHtml(item.reason || "—")} · ${formatDate(item.created_at)}</small></span><div class="admin-row-actions"><button class="admin-row-button" type="button" data-open-player="${escapeHtml(item.blocker_id)}">${english ? "Blocker" : "Bloqueur"}</button><button class="admin-row-button" type="button" data-open-player="${escapeHtml(item.blocked_id)}">${english ? "Blocked" : "Bloqué"}</button></div></div>`
          )
          .join("")
      : empty(english ? "No recent blocks." : "Aucun blocage récent.");
    if (state.social.selected) {
      try {
        await selectSquad(state.social.selected, { silent: true });
      } catch (_) {
  ...
  function renderSquadDossierEmpty() {
    $("#squadDossier").innerHTML =
      `<p class="admin-empty">${escapeHtml(tr("squadPick", "Sélectionnez une squad pour voir membres, invitations, souhaits et activité."))}</p>`;
  }
  ...
  function renderSquadDossier(data) {
    ...
    const stale =
      !squad.last_activity_at || Date.now() - new Date(squad.last_activity_at).getTime() > 30 * 24 * 60 * 60 * 1000;
    const riskyOpen = squad.join_open && Number(squad.member_count) >= 8;
    const signals = [];
    if (riskyOpen)
      signals.push(
        english
          ? "Public join is open on a large squad — review before leaving it open."
          : "L’accès public est ouvert sur une grande squad — à vérifier avant de le laisser ouvert."
      );
    if (stale)
      signals.push(english ? "No activity for more than 30 days." : "Aucune activité depuis plus de 30 jours.");
    if (Number(squad.pending_invite_count) > 0)
      signals.push(
        english
          ? `${formatNumber(squad.pending_invite_count)} pending invitation(s).`
          : `${formatNumber(squad.pending_invite_count)} invitation(s) en attente.`
      );
    $("#squadDossier").innerHTML = `
      ...
      <div class="admin-editor__header">
        ...
        ${stateBadge(squad.join_open ? (english ? "Join open" : "Accès ouvert") : english ? "Join closed" : "Accès fermé", openTone)}
      </div>
