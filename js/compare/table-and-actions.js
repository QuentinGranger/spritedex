...
function renderCompareTable(result, aName, bName) {
  ...
  if ((state.compareHelpFilter || "all") !== "all" && sort === "alpha") {
    records.sort(
      (a, b) =>
        scoreCompareHelp(b).score - scoreCompareHelp(a).score ||
        String(a.spriteName).localeCompare(String(b.spriteName), uiLocale())
    );
  }
  ...
  els.compareTable.querySelectorAll(".compare-action--detail").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      openCompareSprite(btn.dataset.spriteId);
    });
  });
...
function attachCompareQuickActions(container, spriteIdForDialog = null) {
  container.querySelectorAll(".compare-status-select").forEach((sel) => {
    sel.addEventListener("change", (e) => {
      ...
      if (status === "priority")
        logCompareAnalytics("priority_added_from_comparison", {
          variantId: sel.dataset.variantId,
          source: "quick_action"
        });
