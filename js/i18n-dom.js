  ...
  install({ locale, translateLegacy, messageForKey }) {
    ...
    function setKeyedText(element, key) {
      ...
      if (textChildren.length === 1 && element.children.length > 0) {
        textChildren[0].nodeValue = textChildren[0].nodeValue.replace(textChildren[0].nodeValue.trim(), text);
        return;
      }
    ...
    function applyI18n(root = document.body) {
      ...
      scope
        .querySelectorAll(
          "[data-i18n], [data-i18n-placeholder], [data-i18n-title], [data-i18n-aria-label], [data-i18n-alt], [data-i18n-label]"
        )
        .forEach((el) => {
          nodes.push(el);
        });
