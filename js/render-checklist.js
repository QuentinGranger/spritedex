...
function focusChecklistSprite(spriteId) {
  requestAnimationFrame(() => {
    const header = Array.from(els.checklistList?.querySelectorAll("[data-toggle]") || []).find(
      (element) => element.dataset.toggle === String(spriteId)
    );
    header?.focus();
  });
}
...
function sortSprites(sprites) {
  ...
  return [...sprites].sort((a, b) => {
    switch (sort) {
      ...
      case "progress-asc": {
        const pA =
          getVariantList(a).filter((v) => getEntry(variantId(a.id, v)).status === "owned").length /
          (getVariantList(a).length || 1);
        const pB =
          getVariantList(b).filter((v) => getEntry(variantId(b.id, v)).status === "owned").length /
          (getVariantList(b).length || 1);
        return pA - pB;
      }
      case "progress-desc": {
        const pA =
      ...
      case "priority": {
        const bestPrio = (s) =>
          Math.min(...getVariantList(s).map((v) => priorityOrder(getEntry(variantId(s.id, v)).priority || "none")));
        return bestPrio(a) - bestPrio(b);
      }
      case "recent": {
        const latest = (s) =>
          Math.max(
            ...getVariantList(s).map((v) => {
...
function renderChecklist() {
  buildFilterChips();
  const filtered = sortSprites(
    SPRITES.filter((sprite) => getVariantList(sprite).length > 0 && spriteMatchesFilter(sprite))
  );
  ...
  els.checklistList.innerHTML = filtered
    .map((sprite) => {
      ...
      const displayedVariants = variantFilter ? variants.filter((v) => v.name === variantFilter) : variants;
