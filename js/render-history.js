...
function statusLabelFR(status) {
  ...
  return map[status] || (status ? String(status) : "Nouveau");
}
...
function formatHistoryDate(dateStr) {
  ...
  return date ? date.toLocaleDateString("fr-FR", { day: "numeric", month: "short", year: "numeric" }) : "Date inconnue";
}
...
function formatHistoryUpdate(dateStr) {
  ...
  const prefix = sameDay ? "Aujourd’hui" : (date.toDateString() === yesterday.toDateString() ? "Hier" : formatHistoryDate(dateStr));
...
function historyChangeText(item) {
  const from = item.old_status ? statusLabelFR(item.old_status) : "Nouveau";
  return `${from} → ${statusLabelFR(item.new_status)}`;
}
...
function renderHistoryItem(item, index) {
  ...
  const dateText = `${formatHistoryDate(item.created_at)} à ${formatHistoryTime(item.created_at)}`;
  return `
    <article class="history-item history-item--${kind}${isAcquisition(item) ? " history-item--owned" : ""}" role="article" tabindex="0" aria-label="${escapeHtml(`${name}, ${variant}. ${changeText}. ${dateText}`)}" data-history-index="${index}">
      ...
      <div class="history-item__body">
        ...
        <p class="history-item__change"><span>${escapeHtml(statusLabelFR(item.old_status || "new"))}</span><span class="history-item__arrow" aria-hidden="true">→</span><strong>${escapeHtml(statusLabelFR(item.new_status))}</strong></p>
      </div>
      <span class="history-item__type history-item__type--${kind}">${kind === "acquisition" ? "Acquisition" : "Changement"}</span>
...
function renderHistoryKpis() {
  ...
  const cards = [
    { key: "acquisition", icon: "↓", label: "Acquisitions", value: acquisitions, hint: "sur les 12 dernières semaines" },
    { key: "change", icon: "⇄", label: "Changements", value: nonAcquisition, hint: "statuts mis à jour" },
    { key: "streak", icon: "♨", label: "Série actuelle", value: streak, hint: `${streak > 1 ? "semaines actives" : "semaine active"}` },
    { key: "updated", icon: "▣", label: "Dernière mise à jour", value: latest ? formatHistoryDate(latest.created_at) : "—", hint: latest ? formatHistoryTime(latest.created_at) : "Aucune activité" }
  ];
...
function renderWeeklyChart(weeks) {
  ...
  const bars = rows.length ? rows.map(row => {
    const label = row.date.toLocaleDateString("fr-FR", { day: "numeric", month: "short" });
    const pct = Math.max(5, Math.round((row.changes / max) * 100));
    return `<li class="history-trend__bar" style="--history-bar:${pct}%" aria-label="Semaine du ${label} : ${activityLabel(row.changes)}, dont ${row.acquisitions} acquisition${row.acquisitions > 1 ? "s" : ""}"><span class="history-trend__bar-value">${row.changes}</span><span class="history-trend__bar-column" aria-hidden="true"></span><span class="history-trend__bar-label">${label}</span></li>`;
  }).join("") : `<li class="history-trend__empty">Aucune activité au cours des 12 dernières semaines.</li>`;
  return `
    <header class="history-panel__header">
      <div><p class="history-panel__eyebrow">12 dernières semaines</p><h3 id="historyTrendTitle">Évolution de votre collection</h3></div>
      <span class="history-panel__meta">${activityLabel(total)}</span>
    </header>
    <div class="history-trend__plot" role="img" aria-label="${activityLabel(total)} sur les 12 dernières semaines"><ol class="history-trend__bars">${bars}</ol></div>`;
}
...
function renderRecentAcquisitions() {
  ...
  target.innerHTML = `
    <header class="history-panel__header"><div><p class="history-panel__eyebrow">Collection</p><h3 id="historyRecentTitle">Variantes récemment obtenues</h3></div></header>
    ...
    }).join("")}</ul>` : `<p class="history-panel__empty">Vos prochaines acquisitions apparaîtront ici.</p>`}`;
}
...
function renderActionSummary() {
  ...
  // The weekly API aggregate covers twelve weeks. Limit the detail-derived
  // regression segment to that same total so the three donut segments remain
  // mutually exclusive even when only the most recent detail page is loaded.
  ...
  target.setAttribute("aria-busy", "false");
  target.innerHTML = `
    <header class="history-panel__header"><div><p class="history-panel__eyebrow">Sur 12 semaines</p><h3 id="historyActionSummaryTitle">Résumé des actions</h3></div></header>
    <div class="history-action-summary">
      <div class="history-action-summary__ring" style="--history-acquisition:${acquisitionPercent}%;--history-change-end:${changeEndPercent}%" role="img" aria-label="${changes} actions enregistrées"><strong>${changes}</strong><span>actions</span></div>
      <ul class="history-action-summary__legend"><li><span class="history-action-summary__dot history-action-summary__dot--acquisition"></span>Acquisitions <strong>${acquisitions}</strong></li><li><span class="history-action-summary__dot history-action-summary__dot--change"></span>Autres changements <strong>${otherChanges}</strong></li><li><span class="history-action-summary__dot history-action-summary__dot--regression"></span>Obtenu → manquant <strong>${regressions}</strong></li></ul>
    </div>`;
}
...
function renderActiveSprites() {
  ...
  target.setAttribute("aria-busy", "false");
  target.innerHTML = `
    <header class="history-panel__header"><div><p class="history-panel__eyebrow">Événements chargés</p><h3 id="historyActiveSpritesTitle">Collections les plus actives</h3></div></header>
    ${active.length ? `<ol class="history-active__list">${active.map(([spriteId, count], index) => {
      const { name } = spriteName(String(spriteId || ""));
      return `<li><span class="history-active__rank">${index + 1}</span><strong>${escapeHtml(name)}</strong><span class="history-active__bar"><i style="--history-active:${Math.round((count / max) * 100)}%"></i></span><span>${count} action${count > 1 ? "s" : ""}</span></li>`;
    }).join("")}</ol>` : `<p class="history-panel__empty">Les familles les plus modifiées apparaîtront ici.</p>`}`;
}
...
function renderHistoryList() {
  ...
  const totalLabel = historyFilter === "all" ? `${activityLabel(entries.length)} chargée${entries.length > 1 ? "s" : ""}` : `${activityLabel(entries.length)} affichée${entries.length > 1 ? "s" : ""}`;
  if (count) count.textContent = totalLabel;
  list.setAttribute("aria-busy", "false");
  if (!entries.length) {
    list.innerHTML = `<p class="squad-empty">${historyEntries.length ? "Aucune activité ne correspond à ce filtre." : "Aucun historique. Commence à classer tes sprites !"}</p>`;
    return;
  }
  list.innerHTML = entries.map(renderHistoryItem).join("") + (!historyHasMore ? `<p class="history-end">Fin de l'historique</p>` : "");
}
...
async function loadMoreHistory() {
  ...
  try {
    ...
    if (historyOffset === 0) list.innerHTML = `<p class="squad-empty">Erreur réseau. Réessayez dans un instant.</p>`;
  } finally {
    list.setAttribute("aria-busy", "false");
