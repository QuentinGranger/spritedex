...
function sortMissingItems(items) {
  ...
  return [...items].sort((a, b) => {
    if (sort === "priority") {
      const byPriority =
        priorityOrder(missingPriorityValue(getEntry(a.id))) - priorityOrder(missingPriorityValue(getEntry(b.id)));
      if (byPriority) return byPriority;
    } else if (sort === "rarity") {
      const byRarity =
        (rarityOrder[String(a.rarity || "").toLowerCase()] ?? 99) -
...
function getVisibleMissingItems() {
  ...
  const query = String(state.missingSearch || "")
    .trim()
    .toLocaleLowerCase("fr");
  const filter = state.missingFilter || "all";
  return sortMissingItems(
    getReleasedCollectionItems(getAllItems()).filter((item) => {
      ...
      return [item.spriteName, item.variant, item.rarity, item.effect].some((value) =>
        String(value || "")
          .toLocaleLowerCase("fr")
          .includes(query)
      );
...
function renderMissing() {
  ...
  const hasControls =
    Boolean(String(state.missingSearch || "").trim()) ||
  ...
  if (status) {
    ...
    const extra =
      notOwned.length !== allMissing.length
  ...
  const spotted = notOwned.filter(
    (item) => getEntry(item.id).status === "spotted" && !isMissingPriority(getEntry(item.id))
  );
  ...
  [
    [t("missing.section.urgent"), "urgent", urgent],
    [t("missing.section.important"), "important", important],
    [t("missing.section.medium"), "medium", mediumPrio],
    [t("missing.section.low"), "low", lowPrio],
    [t("missing.section.spotted"), "spotted", spotted]
  ].forEach(([title, type, items]) => {
    if (items.length) html += renderMissingSection(title, type, items);
  });
  Object.keys(VARIANT_META).forEach((name) => {
    if (variantGroups[name]?.length) {
      html += renderMissingSection(
        t("missing.section.variants", { name: VARIANT_META[name]?.label || name }),
        name.toLowerCase(),
        variantGroups[name]
      );
      delete variantGroups[name];
    }
  });
  Object.entries(variantGroups).forEach(([name, items]) => {
    if (items.length) html += renderMissingSection(t("missing.section.variants", { name }), "other", items);
  });
...
function renderMissingItem(item) {
  ...
  const prioBadge =
    priority !== "none"
      ? `<span class="farm-item__prio" style="--prio-color:${priorityColor(priority)}">${escapeHtml(priorityLabel(priority))}</span>`
      : "";
...
function copyCurrentMissingList() {
  ...
  const lines = [
    t("missing.copyHeader", { count: items.length, plural }),
    "",
    ...items.map(
      (item) =>
        `- ${item.spriteName} · ${item.variant}${isMissingPriority(getEntry(item.id)) ? ` (${priorityLabel(missingPriorityValue(getEntry(item.id)))})` : ""}`
    )
  ];
