...
function renderSquadDuel(rows, players, items) {
    ...
    if (sectionRows.length > 0) {
      ...
      for (const row of sectionRows) {
        if (row.item.spriteName !== currentSprite) {
          ...
          parts.push(
            `<li class="hunt-list__sprite">${escapeHtml(currentSprite)} <span class="squad-table__rarity">${escapeHtml(localizedRarity(row.item.rarity))}</span></li>`
          );
        }
        parts.push(
          `<li class="hunt-list__item"><span class="hunt-list__variant">${escapeHtml(row.item.variant)}</span></li>`
        );
      }
      parts.push(`</ul>`);
    }
  ...
  if (onlyA.length > 0) {
    buildDuelSection(
      t("squad.duelOnlyA", { a: escapeHtml(pA.name), b: escapeHtml(pB.name) }),
      "→",
      "hunt-section__header--partial",
      onlyA,
      t("squad.duelSubtitle")
    );
  }
  ...
  if (onlyB.length > 0) {
    ...
    buildDuelSection(
      t("squad.duelOnlyB", { a: escapeHtml(pA.name), b: escapeHtml(pB.name) }),
      "←",
      "hunt-section__header--nobody",
      onlyB,
      t("squad.duelSubtitle")
    );
  }
...
function renderSquadSession(players, items) {
  ...
  const toCheck = allDiffs.filter((r) => {
  ...
  parts.push(
    `<span class="session-header__icon"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></span>`
  );
  parts.push(`<div><h3 class="session-header__title">${t("squad.sessionMode")}</h3>`);
  parts.push(
    `<p class="session-header__sub">${t("squad.sessionProgress", { pct: teamPct, found: atLeastOne, total: total })}</p></div>`
  );
  parts.push(`</div>`);
  ...
  if (prioItems.length > 0) {
    ...
    parts.push(
      `<h4 class="session-block__title session-block__title--prio"><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> ${t("squad.prioritiesHeader")}</h4>`
    );
    parts.push(`<ul class="session-list">`);
    for (const r of prioItems.slice(0, 15)) {
      parts.push(`<li class="session-list__item">`);
      parts.push(
        `<span class="session-list__name">${escapeHtml(r.item.spriteName)} <span class="session-list__variant">${escapeHtml(r.item.variant)}</span></span>`
      );
      parts.push(
        `<span class="session-list__meta session-list__meta--missing">${t("squad.missingTo", { count: r.missingCount })}</span>`
      );
      parts.push(`</li>`);
    }
  ...
  if (nobodyItems.length > 0) {
    parts.push(`<div class="session-block">`);
    parts.push(
      `<h4 class="session-block__title session-block__title--nobody"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg> ${t("squad.filterMissingAll")}</h4>`
    );
    parts.push(`<ul class="session-list">`);
  ...
  if (toCheck.length > 0) {
    ...
    for (const r of toCheck.slice(0, 15)) {
      const who = players
        .filter((_, i) => r.statuses[i] === "unsure" || r.statuses[i] === "spotted")
        .map((p) => p.name);
      parts.push(`<li class="session-list__item">`);
      parts.push(
        `<span class="session-list__name">${escapeHtml(r.item.spriteName)} <span class="session-list__variant">${escapeHtml(r.item.variant)}</span></span>`
