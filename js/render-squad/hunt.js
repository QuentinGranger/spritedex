...
function renderSquadHunt(rows, players, items) {
  ...
  if (nobodyRows.length > 0) {
    ...
    parts.push(
      `<span class="hunt-section__icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg></span>`
    );
    ...
    for (const row of nobodyRows) {
      ...
      const prioTag =
        row.priorityBy.length > 0
          ...
          : "";
      parts.push(
        `<li class="hunt-list__item"><span class="hunt-list__variant">${escapeHtml(row.item.variant)}</span>${prioTag}</li>`
      );
    }
    parts.push(`</ul></div>`);
  }
  ...
  if (partialRows.length > 0) {
    ...
    parts.push(
      `<span class="hunt-section__icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 0 0 20" fill="currentColor"/></svg></span>`
    );
    ...
    for (const row of partialRows) {
      ...
      parts.push(
        `<li class="hunt-list__item"><span class="hunt-list__variant">${escapeHtml(row.item.variant)}</span><span class="hunt-owners">${escapeHtml(row.ownedBy.join(", "))}</span></li>`
      );
    }
    parts.push(`</ul></div>`);
  }
  ...
  if (everyoneRows.length > 0) {
    ...
    parts.push(
      `<div class="hunt-section__header hunt-section__header--done hunt-section__toggle" data-toggle="huntEveryoneList">`
    );
    parts.push(
      `<span class="hunt-section__icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>`
    );
    ...
    for (const row of everyoneRows) {
      ...
      parts.push(
        `<li class="hunt-list__item hunt-list__item--done"><span class="hunt-list__variant">${escapeHtml(row.item.variant)}</span></li>`
      );
    }
    parts.push(`</ul></div>`);
  }
