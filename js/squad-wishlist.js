...
function wishlistOptions() {
  return getAllItems()
    .filter((item) => getEntry(item.id).status !== "owned")
    .slice(0, 300)
    .map(
      (item) =>
        `<option value="${escapeHtml(String(item.id))}">${escapeHtml(`${item.spriteName} · ${item.variant}`)}</option>`
    )
    .join("");
}
...
function renderSquadWishlist() {
  ...
  if (!mount) return;
  if (!state.activeSquad || !squadWishlistData) {
    mount.innerHTML = "";
    return;
  }
  ...
  mount.innerHTML = `<header class="squad-wishlist__header"><div><p class="eyebrow">${escapeHtml(t("wishlist.eyebrow"))}</p><h3>${escapeHtml(t("wishlist.title"))}</h3><p>${escapeHtml(t("wishlist.summary", { active: active.length, uncovered, duplicates }))}</p></div></header>
    ...
    <div class="squad-wishlist__list">${
      items.length
        ? items
            .map((item) => {
              const coverage =
                item.coverage === "duplicate"
                  ? t("wishlist.duplicate", { count: item.ownerCount })
                  : item.coverage === "covered"
                    ? t("wishlist.covered")
                    : t("wishlist.uncovered");
            ...
            .join("")
        : `<p class="squad-empty">${escapeHtml(t("wishlist.empty"))}</p>`
    }</div>`;
}
...
async function loadSquadWishlist(code = state.activeSquad) {
  ...
  try {
    ...
    /* The squad itself remains usable when this optional view is unavailable. */
  }
}

async function updateSquadWishlist(itemId, patch) {
  const response = await fetch(
    `${API_BASE}/squads/${encodeURIComponent(state.activeSquad)}/wishlist/${encodeURIComponent(itemId)}`,
    {
      method: "PATCH",
...
function setupSquadWishlistEvents() {
  ...
  mount.addEventListener("submit", async (event) => {
    ...
    if (!variantId) return;
    const response = await fetch(`${API_BASE}/squads/${encodeURIComponent(state.activeSquad)}/wishlist`, {
      method: "POST",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
