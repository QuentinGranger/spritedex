...
const pool = new Pool({ database: "spritedex", host: "localhost", port: 5432 });
...
async function seed() {
  ...
  try {
    ...
    console.log(`✓ ${VARIANT_META.length} variant_meta rows`);
    ...
    console.log(`✓ ${SPRITES.length} sprites rows`);
    ...
    for (const [spriteId, variants] of Object.entries(SPRITE_IMAGES)) {
      for (const [variant, path] of Object.entries(variants)) {
        await client.query(
          ...
          [spriteId, variant, path]
        );
        imgCount++;
      }
    }
    console.log(`✓ ${imgCount} sprite_images rows`);
