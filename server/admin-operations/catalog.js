...
app.get(
  ...
  route(async (req, res) => {
    ...
    const values = [];
    const where = [];
    const normalizedSlug = (alias) =>
      `COALESCE(NULLIF(${alias}.slug, ''), REPLACE(REGEXP_REPLACE(${alias}.id, '^sprite_', ''), '_', '-'))`;
    ...
    if (EDITORIAL_STATUSES.has(status))
      where.push(
        `COALESCE(s.editorial_status, CASE WHEN s.is_released IS FALSE THEN 'draft' ELSE 'published' END) = '${status}'`
      );
    ...
    res.json(
      paged(
        list.rows.map((row) => ({
          ...
          editorialStatus: row.editorial_status || (row.is_released === false ? "draft" : "published"),
          editorialUpdatedAt: row.editorial_updated_at,
          lastVerifiedAt: row.last_verified_at,
          catalogVersion: row.catalog_version,
          variantCount: row.variant_count,
          variantsNeedingReview: row.variants_needing_review,
          sameNameRecords: row.same_name_records
        })),
        count.rows[0]?.count,
        { page, pageSize }
      )
...
app.get(
  ...
  route(async (req, res) => {
    ...
    const [variants, availability, history, spriteImages] = await Promise.all([
      pool.query(
        `SELECT id, sprite_id, variant_type, name, official_name, slug, rarity, image_path, suggested_image_path,
                ...
                FROM sprite_variants WHERE sprite_id = $1 ORDER BY variant_type`,
        [req.params.spriteId]
      ),
      pool.query(
        `SELECT id, start_date, end_date, status, event_id, confidence, data_status, sources, updated_at
                FROM availability_periods WHERE sprite_id = $1 ORDER BY start_date DESC NULLS LAST`,
        [req.params.spriteId]
      ),
      pool.query(
        `SELECT id, entity_type, field, previous_value, new_value, changed_by, changed_at, reason, source_id
                FROM catalog_change_history WHERE entity_id = $1 ORDER BY changed_at DESC LIMIT 20`,
        [req.params.spriteId]
      ),
    ...
    const imageByVariant = new Map(
      spriteImages.rows.map((row) => [String(row.variant || "").toLowerCase(), row.image_path])
    );
    ...
    res.json({
      sprite: sprite.rows[0],
      variants: [...variants.rows, ...compatibilityVariants],
      availabilityPeriods: availability.rows,
...
const spriteEditableFields = {
  catalogId: { column: "catalog_id", limit: 50 },
  slug: { column: "slug", limit: 50 },
  name: { column: "name", limit: 100 },
  officialName: { column: "official_name", limit: 100 },
  rarity: { column: "rarity", limit: 30 },
  color: { column: "color", limit: 60 },
  effect: { column: "effect", limit: 2000 },
  variants: { column: "variants", array: true },
  available: { column: "available", limit: 20 },
  image: { column: "image", url: true, asset: true },
  eventId: { column: "event_id", limit: 100 },
  seasonId: { column: "season_id", limit: 50 },
  dataStatus: { column: "data_status", limit: 20 },
  introducedInUpdate: { column: "introduced_in_update", limit: 20 },
  firstObservedAt: { column: "first_observed_at", date: true },
  lastVerifiedAt: { column: "last_verified_at", date: true },
  officiallyAnnouncedAt: { column: "officially_announced_at", date: true },
  baseSummonCost: { column: "base_summon_cost", integer: true },
  ability: { column: "ability", json: true },
  acquisition: { column: "acquisition", json: true },
  availability: { column: "availability", json: true },
  recurrence: { column: "recurrence", json: true },
  dates: { column: "dates", json: true },
  missingFields: { column: "missing_fields", json: true },
  notes: { column: "notes", json: true },
  sources: { column: "sources", json: true },
  catalogVersion: { column: "catalog_version", limit: 32 },
  isReleased: { column: "is_released", boolean: true }
};
...
function editableUpdates(raw, fields) {
  ...
  for (const [key, rule] of Object.entries(fields)) {
    ...
    if (rule.boolean) value = raw[key] === true;
      ...
      try {
        value =
          raw[key] === "" || raw[key] == null ? [] : typeof raw[key] === "string" ? JSON.parse(raw[key]) : raw[key];
        if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || item.length > 100))
          return { error: `Liste invalide : ${key}` };
      } catch (_) {
        return { error: `JSON invalide : ${key}` };
      }
    } else if (rule.integer) {
      value = raw[key] === "" || raw[key] == null ? null : Number(raw[key]);
      if (value != null && (!Number.isInteger(value) || value < 0)) return { error: `Nombre invalide : ${key}` };
    } else if (rule.decimal) {
      value = raw[key] === "" || raw[key] == null ? null : Number(raw[key]);
      if (value != null && (!Number.isFinite(value) || value < 0 || value > 100))
        return { error: `Nombre invalide : ${key}` };
    } else if (rule.json) {
      if (raw[key] === "" || raw[key] == null) value = null;
      else {
        try {
          value = typeof raw[key] === "string" ? JSON.parse(raw[key]) : raw[key];
        } catch (_) {
          return { error: `JSON invalide : ${key}` };
        }
      }
...
async function updateCatalogEntity({ table, idColumn, id, fields, entityType, body, actor = "unknown" }) {
  ...
  const before = await pool.query(
    `SELECT ${columns.map((column) => `"${column}"`).join(", ")} FROM ${table} WHERE ${idColumn} = $1`,
    [id]
  );
  if (!before.rows.length) return { error: "Élément introuvable", status: 404 };
  const registry = require("../catalog-registry");
  const patch = Object.fromEntries(parsed.updates.map((change) => [change.column, change.value]));
  const updated = await withAdminAudit(
    async (client) => {
      await registry.patchEntity(pool, {
        ...
        entityId: String(id),
        patch,
        source: registry.SOURCES.ADMIN,
        actorLabel: text(actor, 100) || "backoffice",
        reason,
        client
      });
        ...
        await client.query(
         ...
         VALUES ($1, $2, $3, $4::jsonb, $5::jsonb, $6, $7)`,
          [
            ...
            String(id),
            change.key,
            JSON.stringify(before.rows[0][change.column]),
            JSON.stringify(change.value),
            text(actor, 100) || "backoffice",
            reason
          ]
        );
      }
      const row = await client.query(`SELECT * FROM ${table} WHERE ${idColumn} = $1`, [id]);
      return row.rows[0];
    },
    {
      actor,
...
app.patch(
  "/api/admin/catalog/:spriteId",
  requireAdminCapability("catalog.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const result = await updateCatalogEntity({
      table: "sprites",
      idColumn: "id",
      id: req.params.spriteId,
      fields: spriteEditableFields,
      entityType: "sprite",
      body: jsonValue(req.body),
      actor: adminActorFromReq(req)
    });
    if (result.error) return res.status(result.status).json({ error: result.error });
    if (result.releasedChanged)
      syncCatalogueMetaAndFanout().catch((error) => console.error("[admin] catalog fanout:", error.message));
...
const variantEditableFields = {
  name: { column: "name", limit: 100 },
  officialName: { column: "official_name", limit: 100 },
  slug: { column: "slug", limit: 100 },
  rarity: { column: "rarity", limit: 30 },
  imagePath: { column: "image_path", url: true, asset: true },
  suggestedImagePath: { column: "suggested_image_path", url: true, asset: true },
  releaseStatus: { column: "release_status", limit: 20 },
  firstObservedAt: { column: "first_observed_at", date: true },
  summonCost: { column: "summon_cost", integer: true },
  spriteChestDropChancePct: { column: "sprite_chest_drop_chance_pct", decimal: true },
  extraEffectRef: { column: "extra_effect_ref", limit: 50 },
  dataStatus: { column: "data_status", limit: 20 },
  effect: { column: "effect", json: true },
  acquisition: { column: "acquisition", json: true },
  availability: { column: "availability", json: true },
  recurrence: { column: "recurrence", json: true },
  dates: { column: "dates", json: true },
...
app.patch(
  "/api/admin/catalog/variants/:variantId",
  requireAdminCapability("catalog.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const result = await updateCatalogEntity({
      table: "sprite_variants",
      idColumn: "id",
      id: req.params.variantId,
      fields: variantEditableFields,
      entityType: "variant",
      body: jsonValue(req.body),
      actor: adminActorFromReq(req)
    });
...
app.post(
  ...
  adminMutationLimiter,
  route(async (req, res) => {
    const spriteIds = Array.isArray(req.body?.spriteIds)
      ? [...new Set(req.body.spriteIds.map((id) => text(id, 100)).filter(Boolean))].slice(0, 50)
      : [];
    const status = text(req.body?.status, 20);
    const reason = text(req.body?.reason, 1000);
    if (!spriteIds.length || !EDITORIAL_STATUSES.has(status) || !reason)
    ...
    const updated = await withAdminAudit(
      async (client) => {
       ...
       WHERE id = ANY($1::text[]) FOR UPDATE`,
        ...
        const changing = previous.rows.filter(
          (row) => (row.editorial_status || (row.is_released === false ? "draft" : "published")) !== status
        );
        ...
        for (const row of changing) {
          ...
          await registry.patchEntity(pool, {
            ...
            patch: {
              ...
              is_released: status === "published"
            },
            source: registry.SOURCES.ADMIN,
            actorLabel: adminActorFromReq(req),
            reason,
            client
          });
          await client.query(
      ...
      (ids) => ({
        actor: adminActorFromReq(req),
        action: "catalog.bulk_workflow",
        targetType: "catalog",
        targetId: "bulk",
        justification: reason,
        details: { requested: spriteIds.length, updated: ids.length, spriteIds: ids, status, reversiblePerSprite: true }
      })
    );
    syncCatalogueMetaAndFanout().catch((error) =>
      console.error("[admin] bulk catalog workflow fanout:", error.message)
    );
...
app.post(
  "/api/admin/catalog/:spriteId/workflow",
  requireAdminCapability("catalog.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const editorialStatus = text(req.body?.editorialStatus, 20);
    const reason = text(req.body?.reason, 1000);
    if (!EDITORIAL_STATUSES.has(editorialStatus) || !reason)
      return res.status(400).json({ error: "Statut éditorial et justification requis" });
    const registry = require("../catalog-registry");
    const sprite = await withAdminAudit(
      async (client) => {
        const previous = await client.query(
          "SELECT editorial_status, is_released FROM sprites WHERE id = $1 FOR UPDATE",
          [req.params.spriteId]
        );
        if (!previous.rows.length) throw notFound("Sprite introuvable");
        await registry.patchEntity(pool, {
          entityType: "sprite",
          entityId: req.params.spriteId,
          patch: {
            editorial_status: editorialStatus,
            editorial_updated_at: new Date().toISOString(),
            is_released: editorialStatus === "published"
          },
          source: registry.SOURCES.ADMIN,
          actorLabel: adminActorFromReq(req),
          reason,
       ...
       VALUES ('sprite', $1, 'editorialStatus', $2::jsonb, $3::jsonb, $4, $5)`,
          [
            req.params.spriteId,
            JSON.stringify(
              previous.rows[0].editorial_status || (previous.rows[0].is_released === false ? "draft" : "published")
            ),
            JSON.stringify(editorialStatus),
            adminActorFromReq(req),
            reason
          ]
        );
        const result = await client.query(
          `SELECT id, name, editorial_status, editorial_updated_at, is_released FROM sprites WHERE id = $1`,
          [req.params.spriteId]
        );
      ...
      (result) => ({
        actor: adminActorFromReq(req),
        action: `catalog.workflow_${editorialStatus}`,
        targetType: "sprite",
        ...
        details: {
          changes: {
            editorialStatus: {
              before:
                result.previous.editorial_status || (result.previous.is_released === false ? "draft" : "published"),
              after: editorialStatus
            },
            isReleased: { before: result.previous.is_released, after: editorialStatus === "published" }
          }
        }
      })
    );
    if (editorialStatus === "published")
      syncCatalogueMetaAndFanout().catch((error) => console.error("[admin] catalog publish fanout:", error.message));
    res.json({ ok: true, sprite: sprite.sprite });
  })
);

app.post(
  "/api/admin/catalog/variants/:variantId/workflow",
  requireAdminCapability("catalog.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const editorialStatus = text(req.body?.editorialStatus, 20);
    ...
    if (!EDITORIAL_STATUSES.has(editorialStatus) || !reason)
      return res.status(400).json({ error: "Statut éditorial et justification requis" });
    const registry = require("../catalog-registry");
    const variant = await withAdminAudit(
      async (client) => {
        const previous = await client.query(
          "SELECT editorial_status, release_status, sprite_id FROM sprite_variants WHERE id = $1 FOR UPDATE",
        ...
        await registry.patchEntity(pool, {
          ...
          parentSpriteId: previous.rows[0].sprite_id,
          patch: {
            editorial_status: editorialStatus,
            editorial_updated_at: new Date().toISOString()
          },
        ...
        await client.query(
          ...
          [
            req.params.variantId,
            JSON.stringify(previous.rows[0].editorial_status || "published"),
            JSON.stringify(editorialStatus),
            adminActorFromReq(req),
            reason
          ]
        ...
        const result = await client.query(
          "SELECT id, sprite_id, name, editorial_status, editorial_updated_at FROM sprite_variants WHERE id = $1",
          [req.params.variantId]
        );
      ...
      (result) => ({
        actor: adminActorFromReq(req),
        action: `catalog.variant_workflow_${editorialStatus}`,
        targetType: "variant",
        targetId: req.params.variantId,
        justification: reason,
        details: { changes: { editorialStatus: { before: result.before, after: editorialStatus } } }
      })
    );
...
app.post(
  "/api/admin/catalog/:spriteId/history/:historyId/rollback",
  requireAdminCapability("catalog.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    ...
    const registry = require("../catalog-registry");
    const rollback = await withAdminAudit(
      async (client) => {
       ...
       WHERE id = $1 AND entity_type = 'sprite' AND entity_id = $2 FOR UPDATE`,
        ...
        if (!field && !isWorkflow)
          throw new AdminHttpError(400, "Cette entrée ne peut pas être restaurée automatiquement");
        const current = await client.query(
          isWorkflow
            ? "SELECT editorial_status, is_released FROM sprites WHERE id = $1 FOR UPDATE"
            : `SELECT "${field.column}" FROM sprites WHERE id = $1 FOR UPDATE`,
          [req.params.spriteId]
        );
        if (!current.rows.length) throw notFound("Sprite introuvable");
        const restored = entry.previous_value;
        const patch = isWorkflow
          ? {
              editorial_status: restored,
              editorial_updated_at: new Date().toISOString(),
              is_released: restored === "published"
            }
       ...
       VALUES ('sprite', $1, $2, $3::jsonb, $4::jsonb, $5, $6, $7)`,
          [
            ...
            JSON.stringify(
              isWorkflow
                ? current.rows[0].editorial_status || (current.rows[0].is_released === false ? "draft" : "published")
                : current.rows[0][field.column]
            ),
        ...
        const result = await client.query(`SELECT * FROM sprites WHERE id = $1`, [req.params.spriteId]);
        return {
          entry,
          sprite: result.rows[0],
          before: isWorkflow
            ? current.rows[0].editorial_status || (current.rows[0].is_released === false ? "draft" : "published")
            : current.rows[0][field.column],
          after: restored
      ...
      (result) => ({
        ...
        details: {
          ...
          changes: { [result.entry.field]: { before: result.before, after: result.after } }
        }
      })
    );
...
app.post(
  "/api/admin/catalog/:spriteId/availability",
  requireAdminCapability("catalog.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    ...
    if (
      !AVAILABILITY_STATUSES.has(status) ||
    ...
    if (start && end && new Date(start) > new Date(end))
      return res.status(400).json({ error: "Les dates sont incohérentes" });
    const reason = text(body.reason, 1000);
    ...
    const availability = await withAdminAudit(
      async (client) => {
        const result = await client.query(
...
app.get(
  "/api/admin/catalog/:entityType/:entityId/registry",
  requireAdminCapability("catalog.read"),
  route(async (req, res) => {
    const entityType = text(req.params.entityType, 20);
    const entityId = text(req.params.entityId, 100);
    if (!entityType || !entityId || !["sprite", "variant"].includes(entityType)) {
      return res.status(400).json({ error: "entityType sprite|variant et entityId requis" });
    }
    const registry = require("../catalog-registry");
    const [chain, integrity] = await Promise.all([
      registry.reconstructEntity(pool, entityType, entityId),
      registry.verifyEntityChain(pool, entityType, entityId)
    ]);
    res.json({
      ok: true,
      entityType,
      entityId,
      integrity: { ok: integrity.ok, errors: integrity.errors, tipHash: integrity.tipHash, tipSeq: integrity.tipSeq },
      state: chain.state,
...
app.get(
  ...
  route(async (req, res) => {
    ...
    const [list, count] = await Promise.all([
      pool.query(
        `SELECT id, entity_type, entity_id, field, previous_value, new_value, changed_by, changed_at, reason, source_id
                FROM catalog_change_history ${clause} ORDER BY changed_at DESC LIMIT $${values.length - 1} OFFSET $${values.length}`,
        values
      ),
