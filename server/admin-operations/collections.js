...
app.get(
  ...
  requireAdminCapability("collections.read"),
  route(async (_req, res) => {
    const [orphaned, mismatched, invalid, migration, passportQueue, latestErrors, mismatchedEntries] =
      await Promise.all([
        ...
        pool.query(
          `SELECT id, table_name, original_key, error, created_at FROM migration_errors ORDER BY created_at DESC LIMIT 12`
        ),
...
app.post(
  ...
  requireAdminCapability("collections.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const action = text(req.body?.action, 60);
    const reason = text(req.body?.reason, 1000);
    const entryIds = Array.isArray(req.body?.entryIds)
      ? [...new Set(req.body.entryIds.map(numberId).filter(Boolean))].slice(0, 50)
      : [];
    if (action !== "backfill-sprite-references" || !reason)
      return res.status(400).json({ error: "Action et justification requises" });
    const repaired = await withAdminAudit(
      async (client) => {
