...
app.get(
  ...
  requireAdminCapability("notifications.read"),
  route(async (_req, res) => {
    const [queue, deliveries, push, digests, failures, health] = await Promise.all([
      pool.query(
        "SELECT status, COUNT(*)::int AS count FROM notification_delivery_queue GROUP BY status ORDER BY status"
      ),
      pool.query(
        "SELECT channel, status, COUNT(*)::int AS count FROM notification_deliveries GROUP BY channel, status ORDER BY channel, status"
      ),
    ...
    res.json({
      ...
      push: push.rows[0],
      digests: digests.rows[0],
      failedJobs: failures.rows,
...
app.post(
  "/api/admin/notifications/queue/:jobId/retry",
  requireAdminCapability("notifications.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    ...
    await withAdminAudit(
      async (client) => {
       ...
       WHERE id = $1 AND status IN ('failed', 'cancelled') RETURNING id, notification_id`,
          [jobId]
        );
...
app.post(
  ...
  route(async (req, res) => {
    ...
    const jobIds = Array.isArray(req.body?.jobIds)
      ? [...new Set(req.body.jobIds.map(numberId).filter(Boolean))].slice(0, 50)
      : [];
    ...
    const result = await withAdminAudit(
      async (client) => {
        ...
        await client.query(
          `UPDATE notification_delivery_queue SET status = 'pending', attempts = 0, available_at = NULL, not_before = NOW(), processed_at = NULL, last_error = NULL, updated_at = NOW() WHERE id = ANY($1::bigint[])`,
          [ids]
        );
        await client.query(
          `UPDATE notification_deliveries SET status = 'queued', failed_at = NULL, error_code = NULL, error_message = NULL, updated_at = NOW() WHERE notification_id = ANY($1::integer[]) AND status = 'failed'`,
          [notifications]
        );
      ...
      (result) => ({
        actor: adminActorFromReq(req),
        action: "notification.failed_batch_retried",
        targetType: "notification_delivery",
        targetId: null,
        justification: reason,
        details: { retried: result.retried, requestedJobIds: jobIds, limit }
      })
    );
