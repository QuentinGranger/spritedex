...
app.get(
  "/api/admin/passports",
  requireAdminCapability("passports.read"),
  route(async (_req, res) => {
    const [summaries, queues, achievements, visibility] = await Promise.all([
      ...
      pool.query(
        "SELECT status, COUNT(*)::int AS count, MAX(updated_at) AS last_updated FROM passport_recalc_queue GROUP BY status ORDER BY status"
      ),
    ...
    res.json({
      ...
      queue: queues.rows,
      topAchievements: achievements.rows,
...
app.post(
  ...
  requireAdminCapability("passports.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const body = jsonValue(req.body);
    const scope = text(body.scope, 20) || "stale";
    const reason = text(body.reason, 1000);
    if (!reason || !["stale", "all", "player"].includes(scope))
      return res.status(400).json({ error: "Périmètre et justification requis" });
    let users;
    if (scope === "player") {
    ...
    for (const user of users.rows) {
      await enqueuePassportRecalc(user.id, {
        reason: "admin.recalculate",
        triggerEvent: "admin.recalculate",
        notify: false
      });
    }
