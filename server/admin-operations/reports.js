...
app.get(
  ...
  route(async (req, res) => {
    ...
    const [list, count, priorityCounts] = await Promise.all([
      ...
      pool.query(
        `SELECT COUNT(*)::int AS count FROM user_reports WHERE status = $1${countPriorityClause}`,
        countValues
      ),
      pool.query(
    ...
    response.facets = {
      priorityCounts: Object.fromEntries(priorityCounts.rows.map((row) => [row.priority, row.count]))
    };
    res.json(response);
  })
);
...
app.patch(
  ...
  route(async (req, res) => {
    ...
    if (!REPORT_STATUSES.has(status) || status === "open")
      return res.status(400).json({ error: "Statut de traitement invalide" });
    if (!resolution) return res.status(400).json({ error: "Une note de résolution est requise" });
    if (priority && !REPORT_PRIORITIES.has(priority))
      return res.status(400).json({ error: "Priorité de signalement invalide" });
    ...
    const outcome = await withAdminAudit(
      async (client) => {
...
app.post(
  ...
  route(async (req, res) => {
    ...
    if (!APPEAL_STATUSES.has(appealStatus) || appealStatus === "none" || !reason)
      return res.status(400).json({ error: "Recours et justification requis" });
    const report = await withAdminAudit(
      async (client) => {
