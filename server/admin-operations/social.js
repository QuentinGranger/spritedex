...
app.get(
  "/api/admin/social",
  requireAdminCapability("social.read"),
  route(async (req, res) => {
    ...
    const [summary, squads, count, activity, pendingInvites, pendingFriendships, recentBlocks] = await Promise.all([
      ...
      pool.query(
        `SELECT s.id, s.code, s.name, s.join_open, s.visibility, s.created_at, s.created_by,
...
app.get(
  "/api/admin/social/squads/:squadId",
  requireAdminCapability("social.read"),
  route(async (req, res) => {
...
app.patch(
  "/api/admin/social/squads/:squadId/access",
  requireAdminCapability("social.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    const squadId = numberId(req.params.squadId);
    ...
    const squad = await withAdminAudit(
      async (client) => {
...
app.post(
  "/api/admin/social/invitations/:invitationId/cancel",
  requireAdminCapability("social.write"),
  adminMutationLimiter,
  route(async (req, res) => {
    ...
    const invitation = await withAdminAudit(
      async (client) => {
        const result = await client.query(
