...
async function resolveAddressee(reqUser, addresseeId) {
  ...
  if (isNumeric) {
    ...
    result = await pool.query(
      "SELECT id, privacy FROM users WHERE (username = $1 OR username_normalized = LOWER($1)) AND deleted_at IS NULL",
      [id]
    );
  }
...
async function canReceiveFriendRequestFrom(reqUser, targetId) {
  ...
  const recipient = await pool.query("SELECT friend_invites_from FROM users WHERE id = $1 AND deleted_at IS NULL", [
    targetId
  ]);
...
async function getFriendCompletionRate(userId, catalog) {
  ...
  const owned = active.filter(
    (item) => compare.compareServerClassify(collection[item.id] || compare.compareServerDefaultEntry()) === "owned"
  ).length;
  return Math.round((owned / active.length) * 10000) / 100;
}

async function getLastCollectionUpdate(userId) {
  const result = await pool.query("SELECT MAX(updated_at) as last_update FROM sprite_entries WHERE user_id = $1", [
    userId
  ]);
...
async function getFriendPreviewSummary(reqUser, friendId) {
  ...
  if (!result) {
    ...
    const meRes = await pool.query(
      "SELECT id, display_name, username FROM users WHERE id = $1 AND deleted_at IS NULL",
      [reqUser]
    );
    ...
    const userA = {
      id: reqUser,
      displayName: meRes.rows[0]?.display_name || meRes.rows[0]?.username || reqUser,
      collection: collectionA
    };
    const userB = {
      id: friendId,
      displayName: friendRes.rows[0]?.display_name || friendRes.rows[0]?.username || friendId,
