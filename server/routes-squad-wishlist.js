...
async function getWishlistSquad(code) {
  const result = await pool.query("SELECT id, code, name FROM squads WHERE code = $1", [
    String(code || "")
      .trim()
      .toUpperCase()
  ]);
...
async function getVisibleCoverage(squadId, viewerId, variantIds) {
  ...
  const members = await pool.query("SELECT user_id FROM squad_members WHERE squad_id = $1 AND status = 'active'", [
    squadId
  ]);
  const visibleIds = [];
  for (const member of members.rows) {
    if (String(member.user_id) === String(viewerId) || (await canViewCollection(viewerId, member.user_id)))
      visibleIds.push(member.user_id);
  }
...
async function serializeWishlist(squad, viewerId) {
  const [members, items] = await Promise.all([
    pool.query(
      `SELECT sm.user_id, COALESCE(u.display_name, u.username) AS username
      FROM squad_members sm JOIN users u ON u.id = sm.user_id
      WHERE sm.squad_id = $1 AND sm.status = 'active' ORDER BY username`,
      [squad.id]
    ),
    pool.query(
      `SELECT w.*, COALESCE(assignee.display_name, assignee.username) AS assigned_name,
      ...
      WHERE w.squad_id = $1 ORDER BY (w.status = 'wanted') DESC, w.updated_at DESC`,
      [squad.id]
    )
  ]);
  const coverage = await getVisibleCoverage(
    squad.id,
    viewerId,
    items.rows.map((row) => String(row.variant_id))
  );
  return {
...
app.post("/api/squads/:code/wishlist", requireNotSuspended, async (req, res) => {
  ...
  try {
    await pool.query(
      `INSERT INTO squad_wishlist_items (squad_id, variant_id, created_by)
      VALUES ($1, $2, $3) ON CONFLICT (squad_id, variant_id) DO UPDATE SET updated_at = NOW()`,
      [squad.id, variantId, userId]
    );
    broadcastSquadUpdate(userId);
    res.status(201).json(await serializeWishlist(squad, userId));
  } catch (error) {
    console.error("[wishlist create]", error);
    res.status(500).json({ error: "Erreur serveur" });
  }
});
...
app.patch("/api/squads/:code/wishlist/:itemId", requireNotSuspended, async (req, res) => {
  ...
  if (status !== undefined && status !== "wanted" && status !== "found")
  ...
  try {
    const result = await pool.query(
      ...
      SET assigned_to = CASE WHEN $3 THEN $4::integer ELSE assigned_to END,
          ...
          updated_at = NOW()
      WHERE id = $1 AND squad_id = $2 RETURNING id`,
      [req.params.itemId, squad.id, hasAssignment, assignee, status || null, Number(userId)]
    );
