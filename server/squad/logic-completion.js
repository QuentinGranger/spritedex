...
async function getSquadRecommendedGoals(squad, reqUser) {
  ...
  for (const event of eventsResult.rows) {
    ...
    const displayNames = missing
      .slice(0, 5)
      .map((r) => `${r.spriteName} ${r.variantName}`)
      .join(", ");
    ...
    eventGoals.push({
      ...
      target: {
        ...
        variantIds: missing.map((r) => r.variantId),
        names: displayNames + suffix
      },
      participants: visibleMembers.map((m) => ({ userId: m.userId, username: m.username })),
  ...
  eventGoals.sort(
    (a, b) =>
      (levelOrder[a.urgency.level] ?? 4) - (levelOrder[b.urgency.level] ?? 4) ||
      b.expectedCollectiveGain - a.expectedCollectiveGain
  );
  ...
  for (const row of matrix) {
    if (
      row.ownerCount === 0 &&
      row.unknownCount === 0 &&
      compare.classifyRecommendationAvailability(row.availabilityStatus) === "available_now"
    ) {
  ...
  if (topAssignments.length >= 1) {
    ...
    const names = topAssignments
      .slice(0, 5)
      .map((a) => `${a.spriteName} ${a.variantName}`)
      .join(", ");
    ...
    goals.push({
      ...
      currentProgress: 0,
      reason:
        "Ces variantes sont manquantes de toute la squad et les membres sélectionnés sont les mieux placés pour les obtenir.",
...
async function getSquadVersionedCompletionReport(squad, reqUser) {
  ...
  if (excludedPrivateCollections > 0) {
    ...
    warnings.push(
      `Les calculs utilisent ${includedMembers.length} collection${includedMembers.length > 1 ? "s" : ""} sur ${members.length}. ${excludedPrivateCollections} collection${plural ? "s" : ""} privée${plural ? "s" : ""} ${plural ? "sont" : "est"} exclue${plural ? "s" : ""} pour confidentialité.`
    );
  }
  if (unknownCount > activeCatalogue.length * 0.25)
    warnings.push("Plus de 25 % des collections sont inconnues, les statistiques peuvent être sous-estimées.");
...
module.exports = {
  getSquadRecommendedGoals,
  buildSquadCompletionMembers,
  getSquadCompletionScope,
  getSquadVersionedCompletionReport
};
