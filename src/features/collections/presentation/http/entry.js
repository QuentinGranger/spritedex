...
app.put(
  "/api/collection/:userId/:spriteId",
  requireNotSuspended,
  security.validateBody(security.schemas.collectionEntrySchema),
  async (req, res) => {
    ...
    try {
      ...
      const nextMasteryLevel = hasMasteryLevel ? normalizeMasteryLevel({ masteryLevel }, nextStatus) : null;
      ...
      if (!isNewEntry && newStatus !== prevStatus) {
        ...
        if (require("../../../../../server/passport-integrity").isOwnedMissingFlip(prevStatus, newStatus)) {
          require("../../../../../server/passport-integrity")
      ...
      if (
        ...
        scheduleSquadStatsForUser(userId).catch((err) => console.error("[setEntry] squad stats refresh failed", err));
      }
      ...
      if (newStatus === "owned" || prevStatus === "owned") {
        ...
        require("../../../../../server/passport-summary")
          ...
          .catch((err) => console.error("[setEntry] passport recalc schedule failed", err));
      }
      const change = {
        variantId,
        spriteId: baseSpriteId,
