...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      scheduleSquadStatsForUser(userId).catch((err) => console.error("[sync] squad stats refresh failed", err));
      ...
      if (notifyChanges.length) {
        setImmediate(() => {
          ...
          integrity
            ...
            .catch((err) => console.error("[sync] collection_history failed", err));
          integrity
