...
app.post(
  "/api/collection-goals/from-recommendation",
  recommendationGoalLimiter,
  requireNotSuspended,
  async (req, res) => {
    ...
    if (recommendation.participants !== undefined && recommendation.participants !== null) {
      if (
        !Array.isArray(recommendation.participants) ||
        recommendation.participants.length > MAX_RECOMMENDATION_ASSIGNEES
      ) {
    ...
    const rawVariantIds =
      overrides?.variantIds !== undefined && overrides.variantIds !== null
        ? overrides.variantIds
        : target.variantIds !== undefined && target.variantIds !== null
          ? target.variantIds
          : target.variant_id !== undefined && target.variant_id !== null && target.variant_id !== ""
            ? [target.variant_id]
            : [];
    ...
    try {
      ...
      if (squadIdNum) {
        logSquadGoalCreated(squadIdNum, reqUser, title).catch((err) =>
        ...
        analytics.logProductAnalyticsEvent(pool, {
          ...
          details: { goalId: result.id, title, variantIds: cleanVariantIds }
        });
      }
