...
async function deliverExternalChannels(
  pool,
  { notificationId, recipientId, user = {}, targetChannels = [], title, body, url, lang = null }
) {
  ...
  if (targetChannels.includes("push")) {
    ...
    if (Array.isArray(results) && results.length) {
      ...
      if (ok.length) {
        ...
        await deliveries
          .markDeliveryDelivered(pool, notificationId, "push", {
            provider: "web_push",
        ...
        await deliveries
          .markDeliveryFailed(pool, notificationId, "push", {
            provider: "web_push",
  ...
  if (targetChannels.includes("email") && user.email) {
    try {
      ...
      if (emailResult && emailResult.ok) {
        ...
        externalDelivered = true;
        await deliveries
          .markDeliveryDelivered(pool, notificationId, "email", {
            provider: "resend",
        ...
        await deliveries
          .markDeliveryCancelled(pool, notificationId, "email", {
            errorCode: "skipped",
        ...
        await deliveries
          .markDeliveryFailed(pool, notificationId, "email", {
            provider: "resend",
