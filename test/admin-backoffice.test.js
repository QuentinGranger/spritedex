...
const adminClientScripts = [...html.matchAll(/<script src="\/js\/(admin(?:\/[^\"]+)?)\.js"><\/script>/g)].map((match) =>
  path.join(root, "js", `${match[1]}.js`)
);
...
const adminCssFragments = fs
  .readdirSync(adminCssDirectory)
...
const routes = [
  ...
  ...fs
    .readdirSync(adminOperationsDir)
...
for (const endpoint of [
  ...
  "/api/admin/catalog",
  "/api/admin/events",
  "/api/admin/collections/integrity",
  "/api/admin/social",
  "/api/admin/notifications/operations",
  "/api/admin/passports",
  "/api/admin/privacy",
...
assert.match(
  routes,
  /catalog_synced_at|notifications_stuck|passport_synced_at/,
  "overview health must measure freshness and blocked background jobs"
);
assert.match(
  routes,
  /pg_postmaster_start_time|latencyMs/,
  "overview health must measure real database latency and status"
);
assert.match(
  routes,
  /security_migrations[\s\S]*admin_named_operators_v1/,
  "overview health must detect missing required migrations"
);
assert.match(
  fs.readFileSync(path.join(root, "server", "ws.js"), "utf8"),
  /getWebSocketHealth|authFailures|wsHealth/,
  "websocket health must expose real errors and authentication failures"
);
assert.match(
  html,
  /id="overviewHealthCards"|id="overviewHealthStamp"|id="overviewHealthSummary"/,
  "overview must expose a dedicated real-health panel"
);
assert.match(
  client,
  /freshnessLabel|overviewHealthCard|overviewHealthSummary/,
  "overview UI must render freshness and service-health states"
);
assert.match(
  client,
  /data-go-tab=.*tab|overviewHealthCard\(.*"catalog"/,
  "health alerts must link directly to the relevant operational tab"
);
assert.match(routes, /reporter_username/, "reports must expose reporter identity");
assert.match(
  routes,
  /resolved_with_suspension|alsoSuspend|body\.suspend/,
  "moderators must be able to resolve and suspend together"
);
...
assert.match(
  client,
  /closeRelatedOpenReports|reportDecisionCloseRelated/,
  "UI must offer closing related open reports"
);
assert.match(routes, /closeRelatedOpenReports|closedRelated/, "API must support closing related open reports");
assert.match(
  schema,
  /user_reports[\s\S]*priority[\s\S]*appeal_status/,
  "reports must persist priority and appeal lifecycle"
);
assert.match(routes, /REPORT_PRIORITIES|priority = COALESCE/, "moderation must validate and preserve report priority");
assert.match(
  routes,
  /admin_notes|appeal_message|appeal_resolution/,
  "admin notes and appeal decisions must be stored server-side"
);
assert.match(routes, /\/api\/admin\/reports\/:reportId\/appeal/, "appeal processing needs a protected API");
assert.match(
  client,
  /reportDecisionPriority|reportDecisionInternalNote/,
  "moderators must be able to set a priority and private note"
);
assert.match(
  client,
  /handleReportAppeal|data-report-appeal/,
  "player dossiers must expose appeal history and handling"
);
assert.match(
  html,
  /id="reportPriorityFilter"|id="reportTriage"/,
  "report triage must expose an accessible priority filter"
);
...
assert.doesNotMatch(
  client,
  /async function handleReportAction[\s\S]{0,400}window\.prompt/,
  "report decisions must not use prompt dialogs"
);

assert.match(routes, /requireAdminCapability/, "operational endpoints must require the admin session and capability");
assert.match(
  routes,
  /app\.get\("\/api\/admin\/audit", requireAdminCapability\("audit\.read"\)/,
  "the audit journal must have a dedicated protected endpoint"
);
assert.match(
  routes,
  /app\.get\("\/api\/admin\/audit\/export", requireAdminCapability\("audit\.read"\)/,
  "audit export must remain capability-protected"
);
assert.match(
  routes,
  /safeAuditDetails|AUDIT_PRIVATE_DETAIL_KEY/,
  "audit responses must redact sensitive operational fields"
);
...
assert.match(
  schema,
  /suspension_source[\s\S]*'self'[\s\S]*'admin'/,
  "schema must distinguish self-service and admin suspensions"
);
assert.match(
  routes,
  /suspension_source\s*=\s*CASE WHEN \$2 THEN 'admin'/,
  "admin suspensions must be marked as admin-owned"
);
...
assert.match(
  routes,
  /INSERT INTO admin_audit_log[\s\S]*player\.suspended|writeAdminAudit[\s\S]*player\.suspended/,
  "suspension and audit history must be written together"
);
assert.match(
  profileRoutes,
...
assert.match(
  authRoutes,
  /adminSuspended[\s\S]*Compte suspendu par un administrateur/,
  "admin-suspended users must not receive new login sessions"
);
...
assert.match(
  adminAccess,
  /ADMIN_OPERATOR_LABEL|resolveOperatorLabel/,
  "operators must be attributable beyond a shared terminal label"
);
assert.match(
  adminAccess,
  /max_expires_at|ADMIN_SESSION_MAX_TTL_MS/,
  "admin sessions must have an absolute expiry ceiling"
);
assert.match(
  adminAccess,
  /ADMIN_MAX_CONCURRENT_SESSIONS|enforceConcurrentSessionLimit/,
  "concurrent admin sessions must be capped"
);
assert.match(
  adminAccess,
  /session\.context_changed|ipChanged/,
  "admin sessions must audit material client-context changes"
);
assert.match(
  schema,
  /admin_operators|admin_security_alerts/,
  "named admin accounts and persistent security alerts must be stored durably"
);
assert.match(
  adminAccess,
  /verifyAdminOperatorCredentials|rotateAdminOperatorSecret/,
  "named admin credentials and secret rotation must be implemented server-side"
);
assert.match(
  fs.readFileSync(path.join(root, "server", "routes-admin.js"), "utf8"),
  /terminalIdentityLimiter|keyGenerator/,
  "admin login attempts must be limited per address and identity"
);
assert.match(
  fs.readFileSync(path.join(root, "server", "routes-admin.js"), "utf8"),
  /\/api\/admin\/operators|security-alerts/,
  "owner operations must manage named admins and security alerts"
);
assert.match(
  adminAccess,
  /unusualLogin|security\.unusual_login/,
  "unusual named-admin logins must generate an auditable alert"
);
assert.match(
  html,
  /id="adminOperators"|id="adminSecurityAlerts"/,
  "admin access and security alerts must be visible in the backoffice"
);
assert.match(
  client,
  /openAdminOperatorDialog|toggleAdminOperator|acknowledgeSecurityAlert/,
  "backoffice must create, rotate and control named admin access"
);
...
assert.match(
  routes,
  /Événement et justification requis|Actualité et justification requises/,
  "event and news creation must require a justification"
);
...
assert.match(
  ...
  "catalog workflow states must be stored and validated server-side"
);
assert.match(
  routes,
  /catalog\/:spriteId\/workflow|catalog\.workflow_/,
  "sprites must support auditable editorial transitions"
);
assert.match(
  routes,
  /catalog\/bulk-workflow|catalog\.bulk_workflow/,
  "catalog must support bounded auditable bulk workflow updates"
);
assert.match(
  routes,
  /events\/bulk-status|event\.bulk_status/,
  "events must support bounded auditable bulk status updates"
);
assert.match(
  html,
  /id="catalogBulkBar"|id="eventsBulkBar"|id="bulkActionDialog"/,
  "bulk actions need selection controls and a confirmation dialog"
);
assert.match(
  client,
  /openBulkAction|submitBulkAction|bulkIds/,
  "client must preview and execute selected bulk actions"
);
assert.match(routes, /variants\/:variantId\/workflow|variant_workflow_/, "variants must support editorial transitions");
assert.match(
  routes,
  /history\/:historyId\/rollback|catalog\.rollback/,
  "catalog history must support audited rollback"
);
assert.match(
  routes,
  /withAdminAudit[\s\S]*report\.\$\{status\}|withAdminAudit[\s\S]*report\./,
  "report resolutions must audit atomically"
);
assert.match(
  routes,
  /notification\.queue_process_requested/,
  "queue flushes must audit the decision before side effects"
);
...
assert.match(
  graphAdmin,
  /graph\.metric_suspended|graph\.metric_restored/,
  "sprite graph flag audits must name the action"
);
...
assert.match(
  client,
  /loadAudit|openAuditDetail|exportAudit/,
  "the client must load, inspect and export the audit journal"
);
...
assert.doesNotMatch(
  client,
  /async function handlePlayerAction[\s\S]{0,500}window\.prompt/,
  "player suspension must not use prompt dialogs"
);
...
assert.match(
  client,
  /setCatalogWorkflow|setVariantWorkflow|rollbackCatalogHistory/,
  "client must drive catalog workflow and rollback actions"
);
assert.match(client, /retryFailedNotifications/, "client must support batch notification recovery");
assert.match(
  routes,
  /notifications\/retry-failed|failed_batch_retried/,
  "notification failures must support auditable batch retry"
);
assert.doesNotMatch(
  client,
  /function createEvent[\s\S]{0,900}window\.prompt/,
  "event creation must not use prompt dialogs"
);
assert.doesNotMatch(
  client,
  /function createNews[\s\S]{0,900}window\.prompt/,
  "news creation must not use prompt dialogs"
);
assert.doesNotMatch(
  client,
  /function openVariantEditor[\s\S]{0,600}window\.prompt/,
  "variant edits must not use prompt dialogs"
);
assert.doesNotMatch(
  client,
  /function openAvailabilityEditor[\s\S]{0,600}window\.prompt/,
  "availability creation must not use prompt dialogs"
);
assert.doesNotMatch(
  client,
  /async function editEvent[\s\S]{0,500}window\.prompt/,
  "event edits must not use prompt dialogs"
);
assert.doesNotMatch(
  client,
  /async function updateNewsStatus[\s\S]{0,500}window\.prompt/,
  "news publish/archive must not use prompt dialogs"
);
...
assert.match(
  client,
  /willFanoutNews|syncNewsEditorChrome|refreshNewsPreview/,
  "client must preview news and detect first publish"
);
assert.match(
  client,
  /previousStatus|forceStatus/,
  "publish actions must preserve the prior status for fan-out detection"
);
assert.match(
  client,
  /name="color"|name="eventId"|name="seasonId"|name="effect"/,
  "catalog editor must expose the editable sprite fields"
);
assert.match(
  routes,
  /SELECT COUNT\(\*\).*FROM squad_members sm[\s\S]*WHERE sm\.squad_id = s\.id/,
  "squad member counts must not use cartesian JOIN inflation"
);
assert.doesNotMatch(
  routes,
...
assert.match(
  client,
  /openSquadInviteCancelDialog|submitSquadInviteCancel/,
  "client must drive invitation cancellation"
);
assert.doesNotMatch(
  client,
  /function openSquadAccessDialog[\s\S]{0,200}window\.prompt|async function toggleSquad[\s\S]{0,400}window\.prompt/,
  "squad access must not use prompt dialogs"
);
...
assert.match(
  routes,
  /privacy\.export_generated|privacy\.accounts_purged|privacy\.account_purged/,
  "privacy mutations must be audited"
);
assert.match(
  fs.readFileSync(path.join(root, "server", "privacy-ops.js"), "utf8"),
  /async function purgeDeletedAccounts/,
  "purge logic must be centralized"
);
...
assert.match(
  client,
  /privacyPurgeConfirm|Type .* to confirm|Tapez/,
  "purge confirmation must require typing the username"
);
...
assert.match(
  routes,
  /privacy\.account_restored|privacy\.share_links_revoked/,
  "restore and link revocation must be audited"
);
assert.match(routes, /app\.get\("\/api\/admin\/search"/, "backoffice must expose a universal search endpoint");
assert.match(
  routes,
  /squad_invitations|notification_delivery_queue|admin_audit_log/,
  "universal search must cover operational records beyond catalog"
);
assert.match(
  routes,
  /friend_invite_links/,
  "universal search must include friend invitation links without exposing their token"
);
assert.match(
  html,
  /id="adminUniversalSearch"|id="adminSearchDialog"/,
  "backoffice must expose the universal search palette"
);
assert.match(
  client,
  /openUniversalSearch|searchUniversally|openUniversalResult/,
  "client must provide universal search interactions"
);
assert.match(
  client,
  /moveUniversalSelection|data-universal-filter/,
  "universal search must support keyboard navigation and result filters"
);
assert.match(
  client,
  /universalQuickActions|renderUniversalQuickActions/,
  "universal search must provide safe quick actions before a query"
);
assert.match(
  routes,
  /sv_preview\.image_path|si_preview\.image_path/,
  "admin catalog lists must fall back to variant images when a sprite image is absent"
);
assert.match(client, /adminCatalogImage|adminImageUrl/, "admin catalog UI must normalize local sprite image paths");
assert.match(
  routes,
  /same_name_records|data_issues/,
  "admin catalog must surface duplicate candidates and isolated records as data issues"
);
assert.match(
  routes,
  /canonical\.id LIKE 'sprite\\\\_%'|compatibilityVariants/,
  "admin catalog must hide legacy short ids from the primary view and support seeded variants"
);
assert.match(
  client,
  /renderCatalogEmptyState|data-catalog-show-issues/,
  "catalog inspector must guide admins toward data issues before a selection"
);
assert.match(
  css,
  /\.admin-bulk-bar\[hidden\].*display: none !important/,
  "hidden bulk actions must never occupy the catalog layout"
);
...
assert.match(
  authz,
  /ROLE_CAPABILITIES|resolveOperatorRole|hasCapability/,
  "admin AuthZ must define roles and capabilities"
);
...
assert.match(
  adminAccess,
  /consumeTotpCode|AdminAccessError|peekAdminTicket/,
  "ticket consume must support MFA without burning the ticket on failure"
);
assert.match(adminRoutes, /requireAdminCapability/, "session routes must expose capability middleware");
assert.match(
  adminRoutes,
  /\/api\/admin\/terminal\/challenge/,
  "access page must be able to challenge a ticket before MFA"
);
assert.match(
  adminRoutes,
  /requireAdminStepUp|ADMIN_STEPUP_REQUIRED/,
  "destructive privacy actions must require MFA step-up"
);
...
assert.match(
  ...
  "moderation writes must require players.moderate"
);
...
assert.match(
  graphAdmin,
  /intelligence\.write|intelligence\.read/,
  "sprite graph terminal access must honor intelligence capabilities"
);
...
assert.match(
  client,
  /function can\([\s\S]*return false/,
  "capability checks must fail closed before session hydration"
);
