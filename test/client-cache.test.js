...
assert.ok(assets.includes("/js/i18n-nl-legacy-data.js"), "le dictionnaire néerlandais historique doit participer à l'empreinte");
assert.ok(assets.includes("/js/legal-content-nl.js"), "les documents juridiques néerlandais doivent participer à l'empreinte");
assert.ok(worker.includes(`const CACHE_NAME = "${cacheVersion(root, assets)}"`), "version de cache absente du worker généré");
...
assert.match(eventSource, /register\("\/sw\.js", \{ scope: "\/", updateViaCache: "none" \}\)/, "le worker doit être enregistré à la racine sans réutiliser un cache HTTP");
