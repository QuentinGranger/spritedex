  ...
  await test("suspended account hides profile and blocks new invitations while keeping friendships", async () => {
    ...
    try {
      ...
      let res = await fetch(`${API}/friends/requests`, {
        ...
        body: JSON.stringify({ addresseeId: paul.id })
      });
      ...
      assert.ok(
        friends.friends.some((f) => f.id === nina.id),
        "nina missing before suspend"
      );

      // Nina suspends her account.
      res = await fetch(`${API}/profile/${nina.id}/suspend`, {
        method: "POST",
        headers: auth(nina.token),
      ...
      assert.ok(
        friends.friends.some((f) => f.id === nina.id),
        "nina should reappear after unsuspend"
      );
