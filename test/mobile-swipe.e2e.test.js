...
const MOBILE_PROFILES = Object.freeze({
  iphoneSE: { name: "iPhone SE", viewport: { width: 375, height: 667, isMobile: true, hasTouch: true, deviceScaleFactor: 2 } },
  iphone13: { name: "iPhone 13", viewport: { width: 390, height: 844, isMobile: true, hasTouch: true, deviceScaleFactor: 3 } },
  android: { name: "Android", viewport: { width: 412, height: 915, isMobile: true, hasTouch: true, deviceScaleFactor: 2.625 } }
});
const DESKTOP_PROFILE = Object.freeze({ name: "Desktop", viewport: { width: 1440, height: 900, isMobile: false, hasTouch: false, deviceScaleFactor: 1 } });
...
async function cardSignature(page) {
  return page.evaluate(() => ["cardName", "cardVariant", "cardIndex"]
...
async function openGuestApp(browser, profile, initialView = "swipe") {
  ...
  const catalogueLoaded = page.waitForResponse((response) => {
    try {
  ...
  if (initialView === "swipe") {
    ...
    await page.waitForFunction(() => {
      ...
      return card && !card.classList.contains("is-refreshing")
...
async function main() {
  ...
  try {
    ...
    await test("l'accueil met en avant une recommandation intelligente et ouvre le tri", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      ...
      await page.waitForFunction(() => document.querySelector(".tab.active")?.dataset.view === "swipe", { timeout: CARD_READY_TIMEOUT });
    }, "home"));

    await test("l'état de sauvegarde reste explicite hors ligne", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      assert.ok(await page.$eval("#syncBar", (bar) => bar.classList.contains("sync-bar--local")), "état local absent");
      await page.evaluate(() => {
        state.userId = "sync-test";
        Object.defineProperty(Navigator.prototype, "onLine", { configurable: true, get: () => false });
        window.dispatchEvent(new Event("offline"));
      });
      await page.waitForFunction(() => document.getElementById("syncBar")?.classList.contains("sync-bar--offline"), { timeout: CARD_READY_TIMEOUT });
      await page.evaluate(() => {
        state.userId = null;
        Object.defineProperty(Navigator.prototype, "onLine", { configurable: true, get: () => true });
        window.dispatchEvent(new Event("online"));
      });
      await page.waitForFunction(() => document.getElementById("syncBar")?.classList.contains("sync-bar--local"), { timeout: CARD_READY_TIMEOUT });
    }, "home"));

    await test("la navigation mobile compacte les vues secondaires dans Plus", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      assert.strictEqual(await page.$eval('.tab[data-view="stats"]', (tab) => getComputedStyle(tab).display), "none");
      await page.evaluate(() => document.getElementById("mobileMoreButton")?.click());
      await page.waitForFunction(() => !document.getElementById("mobileMoreMenu")?.hidden, { timeout: CARD_READY_TIMEOUT });
      ...
      await page.waitForFunction(() => document.querySelector(".tab.active")?.dataset.view === "stats", { timeout: CARD_READY_TIMEOUT });
      assert.ok(await page.$eval("#mobileMoreButton", (button) => button.classList.contains("active")));
    }, "home"));

    await test("la recherche globale tactile trouve une variante et permet de la marquer possédée", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      ...
      await page.waitForFunction(() => document.getElementById("commandPalette")?.open, { timeout: CARD_READY_TIMEOUT });
      await page.type("#commandPaletteInput", target.query);
      await page.waitForFunction(() => [...document.querySelectorAll("[data-command-index]")]
        .some((button) => button.textContent.includes("Marquer possédé")), { timeout: CARD_READY_TIMEOUT });
      await page.evaluate(() => [...document.querySelectorAll("[data-command-index]")]
        .find((button) => button.textContent.includes("Marquer possédé"))?.click());
      await page.waitForFunction((itemId) => getEntry(itemId).status === "owned", { timeout: CARD_READY_TIMEOUT }, target.id);
      assert.ok(await page.$eval("#commandPalette", (dialog) => !dialog.open), "la palette doit se fermer après l’action");
    }, "home"));

    await test("la recherche globale comprend les intentions de navigation et de réglages", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      await touchTap(page, "#commandPaletteOpen");
      await page.type("#commandPaletteInput", "statistiques");
      await page.waitForFunction(() => [...document.querySelectorAll("[data-command-index]")]
        .some((button) => button.textContent.includes("Ouvrir les statistiques")), { timeout: CARD_READY_TIMEOUT });
      await page.evaluate(() => [...document.querySelectorAll("[data-command-index]")]
        .find((button) => button.textContent.includes("Ouvrir les statistiques"))?.click());
      await page.waitForFunction(() => document.querySelector(".tab.active")?.dataset.view === "stats", { timeout: CARD_READY_TIMEOUT });
      await page.evaluate(() => document.getElementById("commandPaletteOpen")?.click());
      await page.type("#commandPaletteInput", "reglages");
      await page.waitForFunction(() => [...document.querySelectorAll("[data-command-index]")]
        .some((button) => button.textContent.includes("Ouvrir mon compte")), { timeout: CARD_READY_TIMEOUT });
    }, "home"));

    await test("desktop : la palette se ferme par Échap et par sa croix", () => withGuestApp(browser, DESKTOP_PROFILE, async (page) => {
    ...
    await test("le plan de farm par événement est accessible depuis les manquants", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      await page.evaluate(() => document.querySelector('.tab[data-view="missing"]')?.click());
      await page.waitForFunction(() => document.querySelector(".tab.active")?.dataset.view === "missing", { timeout: CARD_READY_TIMEOUT });
      assert.ok(await page.$("#farmPlanner .farm-planner__heading"), "en-tête du plan de farm absent");
    }, "home"));

    await test("une priorité déclarée devient la recommandation principale", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      ...
      await page.waitForFunction((name) => document.querySelector("#nowPrimary h3")?.textContent.includes(name), { timeout: CARD_READY_TIMEOUT }, targetName);
    }));

    await test("la session de tri suit la progression, se met en pause et annule la dernière action", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      ...
      await waitForCardChange(page, before);
      await page.waitForFunction(() => document.getElementById("swipeSessionProgress")?.textContent.includes("1 traité"), { timeout: CARD_READY_TIMEOUT });
      await touchTap(page, "#swipeSessionPause");
      await page.waitForFunction(() => document.getElementById("swipeSessionPause")?.textContent.includes("Reprendre") && document.getElementById("markOwned")?.disabled, { timeout: CARD_READY_TIMEOUT });
      await touchTap(page, "#swipeSessionPause");
      await touchTap(page, "#swipeSessionUndo");
      await page.waitForFunction(({ id, status }) => getEntry(id).status === status, { timeout: CARD_READY_TIMEOUT }, target);
      assert.ok((await page.$eval("#swipeSessionProgress", (node) => node.textContent)).includes("0 traité"));
    }));

    await test("le poste de tri mobile tient au-dessus de la navigation sans défilement", () => withGuestApp(browser, MOBILE_PROFILES.iphoneSE, async (page) => {
      ...
      assert.ok(layout.actionsBottom <= layout.navTop - 4, `actions recouvertes par la navigation (${layout.actionsBottom} > ${layout.navTop})`);
    }));

    await test("desktop : les raccourcis de session classent puis annulent", () => withGuestApp(browser, DESKTOP_PROFILE, async (page) => {
      const target = await page.evaluate(() => ({ id: currentItem().id, status: getEntry(currentItem().id).status }));
      const before = await cardSignature(page);
      await page.keyboard.press("?");
      await page.waitForFunction(() => !document.getElementById("swipeShortcutGuide")?.hidden, { timeout: CARD_READY_TIMEOUT });
      await page.keyboard.press("Escape");
      await page.waitForFunction(() => document.getElementById("swipeShortcutGuide")?.hidden, { timeout: CARD_READY_TIMEOUT });
    ...
    await test("iPhone SE : swipe droit conserve le scroll et change de carte", () => withGuestApp(browser, MOBILE_PROFILES.iphoneSE, async (page) => {
      ...
      assert.ok(Math.abs(scrollAfter - scrollBefore) <= 3, `scroll déplacé de ${scrollAfter - scrollBefore}px après swipe droit`);
    }));

    await test("Android : swipe vertical vers le haut conserve le scroll et change de carte", () => withGuestApp(browser, MOBILE_PROFILES.android, async (page) => {
      await cardCenter(page);
      const scrollBefore = await page.evaluate(() => window.scrollY);
      const before = await cardSignature(page);
      ...
      await waitForCardChange(page, before);
      const scrollAfter = await page.evaluate(() => window.scrollY);
      assert.ok(Math.abs(scrollAfter - scrollBefore) <= 3, `scroll déplacé de ${scrollAfter - scrollBefore}px après swipe vertical`);
    }));
    ...
    await test("la nouvelle carte est disponible rapidement après une action", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      const before = await cardSignature(page);
    ...
    await test("un swipe diffère les listes cachées puis les rafraîchit à l'ouverture", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
      ...
      assert.strictEqual(await page.$eval("#checklistList", (list) => list.innerHTML), beforeChecklist, "checklist reconstruite pendant le swipe");
      assert.strictEqual(await page.$eval("#missingList", (list) => list.innerHTML), beforeMissing, "manquants reconstruits pendant le swipe");
      assert.strictEqual(await page.$eval("#rarityBars", (list) => list.innerHTML), beforeStats, "stats reconstruites pendant le swipe");
      await page.evaluate(() => document.querySelector('.tab[data-view="checklist"]')?.click());
      await page.waitForFunction((previous) => document.getElementById("checklistList")?.innerHTML !== previous, { timeout: CARD_READY_TIMEOUT }, beforeChecklist);
    }));

    await test("le swipe reste utilisable après un retour d'arrière-plan", () => withGuestApp(browser, MOBILE_PROFILES.iphone13, async (page) => {
