...
async function run({ owner, friend, stranger }) {
  ...
  await test("accomplissements persistants + record historique (Étape 16)", async () => {
    ...
    assert.ok(
      ...
      "first_collection should unlock"
    );
    ...
    assert.ok(
      pass.data.badgeProgress.every((b) => b.uiCategory),
      "Étape 63 — uiCategory"
    );
    assert.ok(
      pass.data.recentActivity.some((a) => a.activityType === "account_created" || a.type === "account_created")
    );
