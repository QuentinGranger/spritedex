...
module.exports = {
  ...
  async run() {
    ...
    assert.strictEqual(GRAPH_EVENT_VERSIONS[GRAPH_EVENT_TYPES.COMPARISON_COMPLETED], 2);
    ...
    const effective = await pool.query(`SELECT id FROM graph_events_effective WHERE id = $1::uuid`, [bad.id]);
