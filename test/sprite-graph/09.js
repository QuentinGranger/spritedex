...
module.exports = {
  ...
  async run() {
    ...
    assert.strictEqual(buildGoalCompletedContext({ goal: { scope: "friends" } }).goalScope, "friends");

    assert.strictEqual(FUTURE_GRAPH_EVENT_TYPES.NOTIFICATION_ACTION_CLICKED, "notification.action_clicked");
    assert.strictEqual(FUTURE_GRAPH_EVENT_TYPES.NOTIFICATION_CONVERTED, "notification.converted");
    ...
    await pool.query(`UPDATE notifications SET delivered_at = NOW() - INTERVAL '3 minutes' WHERE id = $1`, [notif.id]);
    ...
    assert.ok(
      ctx.destination === "/sprites/batman?holofoil" || ctx.destination == null || typeof ctx.destination === "string"
    );
