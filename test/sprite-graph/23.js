...
module.exports = {
  ...
  async run() {
    ...
    const first = await recordCollectionGraphEvents(
      user.id,
      [
        {
    ...
    const imported = await recordCollectionGraphEvents(
      user.id,
      [
        {
    ...
    await pool.query(`DELETE FROM sprite_entries WHERE user_id = $1 AND variant_id = $2`, [user.id, variantId]);
    ...
    if (afterApi.rows.length) {
      ...
      assert.ok(afterApi.rows[0].source === "api" || afterApi.rows[0].context?.origin === "collection.setEntry");
    }
