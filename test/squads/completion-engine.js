...
module.exports = async function runCompletionEngine() {
  await test("Squad Completion Engine report + simulate (contrat moteur)", async () => {
    const owner = await register(`EngOwn${rnd()}`),
      mate = await register(`EngMate${rnd()}`);
    try {
      ...
      const uniqueVariant = samples.activeId,
        missingVariant = samples.secondActiveId || samples.activeIds[1];
      ...
      for (const field of [
        ...
        assert.ok(report[field], `${field} required`);
      ...
      const simRes = await fetch(simulateUrl, {
        method: "POST",
        headers: auth(owner.token),
        body: JSON.stringify({ changes: [{ type: "acquire", memberId: mate.id, variantIds: [missingVariant] }] })
      });
      ...
      const oversizedChanges = await fetch(simulateUrl, {
        method: "POST",
        headers: auth(owner.token),
        body: JSON.stringify({
      ...
      assert.strictEqual(
        oversizedChanges.status,
        400,
        `oversized simulation should fail: ${await oversizedChanges.text()}`
      );
      const oversizedVariants = await fetch(simulateUrl, {
        method: "POST",
        headers: auth(owner.token),
        body: JSON.stringify({
          changes: [{ type: "acquire", memberId: mate.id, variantIds: Array(101).fill(missingVariant) }]
        })
      });
      assert.strictEqual(
        oversizedVariants.status,
        400,
        `oversized variant list should fail: ${await oversizedVariants.text()}`
      );
      ...
      assert.strictEqual(
        malformedKick.status,
        400,
        `malformed targetUserId should fail: ${await malformedKick.text()}`
      );
    } finally {
      await cleanup(owner);
      await cleanup(mate);
