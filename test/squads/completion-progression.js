...
module.exports = async function runCompletionProgression(samples) {
  await test("squad_completion_increased progression (Étape 65)", async () => {
    const owner = await register(`SqE65A${rnd()}`),
      mate = await register(`SqE65B${rnd()}`),
      leaver = await register(`SqE65C${rnd()}`);
    ...
    try {
      ...
      const hit = notifs.notifications.find(
        (n) => n.type === "squad_completion_increased" && String(n.actor_id || n.actor?.id) === String(mate.id)
      );
      ...
      const mateBefore = (
        await (await fetch(`${API}/notifications`, { headers: auth(mate.token) })).json()
      ).notifications.filter(
        (n) => n.type === "squad_completion_increased" && String(n.actor_id || n.actor?.id) === String(owner.id)
      ).length;
      ...
      const ownerAfter = (
        await (await fetch(`${API}/notifications`, { headers: auth(owner.token) })).json()
      ).notifications.filter((n) => n.type === "squad_completion_increased").length;
      ...
      const rapid = notifs.notifications.filter(
        (n) =>
          n.type === "squad_completion_increased" &&
          n.read_at == null &&
      ...
      assert.ok(
        Array.isArray(rapid[0].data?.newVariantIds) && rapid[0].data.newVariantIds.length >= 2,
      ...
      assert.ok(
        !leaverNotifs.notifications.some((n) => n.type === "squad_completion_increased" && n.read_at == null),
