const { assert, test, rnd, register, cleanup, createSquad, joinSquad, sendFriendRequest, acceptFriendRequest, getSquadRecommendations, getSquadCompletion, setPrivacy, setEntry, resetCollection, blockUser, unblockUser } = require("./shared");

module.exports = async function runRecommendations(samples) {
  const henry = await register(`SqHenry${rnd()}`), irene = await register(`SqIrene${rnd()}`);
  try {
    ...
    await test("les contenus non sortis sont exclus des recommandations", async () => {
      ...
      const candidate = (await getSquadRecommendations(henry.token, squad.code)).recommendations.friendsToInvite.find(c => String(c.userId) === String(irene.id));
      assert.ok(candidate, "friend not recommended");
      assert.ok(candidate.newVariantsForSquad >= 1, "active variant should contribute");
      if (samples.unreleasedId) assert.ok(candidate.potentialContribution <= candidate.newVariantsForSquad, "unreleased variant leaked into contribution");
    });
    ...
    await test("les pourcentages sont recalculés après une modification", async () => {
      ...
      if (after) {
        ...
        assert.ok(after.currentCompletionRate > before.currentCompletionRate || after.projectedCompletionRate > before.projectedCompletionRate, "rates should increase");
      }
  ...
  } finally { await cleanup(henry); await cleanup(irene); }
};
